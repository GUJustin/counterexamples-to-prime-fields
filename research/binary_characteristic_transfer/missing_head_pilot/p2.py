...
def mul(a,b):
 ...
 while b:
  ...
  if a&128:a^=mod
 return c
M=[[mul(a,b) for b in range(128)] for a in range(128)]
def pw(a,n):
 r=1
 while n:
  if n&1:r=M[r][a]
  a=M[a][a];n>>=1
 return r
inv=[0]+[pw(a,126) for a in range(1,128)]
assert all(M[a][inv[a]]==1 for a in range(1,128))
def loc(rows):
 c=[1]
 for x in rows:
  z=0;y=x
...
result={'p':2,'field_modulus':mod,'dimension':7,'subspace_dimension':4,'subspaces':count,'head_lines':len(G),'universal_subspaces':len(universal),'impossible_subspaces':impossible,'group_size_histogram':dict(collections.Counter(map(len,G.values()))),'label_histogram':dict(collections.Counter(r['labels'] for r in out)),'best':out[:12],'elapsed_seconds':time.time()-start}
assert count==11811
