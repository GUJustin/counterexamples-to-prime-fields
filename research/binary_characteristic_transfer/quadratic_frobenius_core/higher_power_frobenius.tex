...
\input{research/binary_characteristic_transfer/quadratic_frobenius_core/odd_h_exact_source_lemma.tex}
...
\[
 ...
 \end{gathered}
\]
Set $U=hp$ if $p\ge3h-1$, and $U=hp+h^2$ otherwise. Suppose that
...
Both endpoints have empty threshold-$T$ lists. If $p\ge3h-1$, every
...
that every such $q$ has at most $hp+h^2$ matches. When $p\ge3h-1$,
...
$p\ge3h-1$, so $U=hp$, and $(h+R)(p-1)\sim2hp$. The branch condition also holds.
...
The sharper characteristic condition also permits $p=307$, $h=65$,
and $m=17h(p^2-1)/20$, where $p<h^2$. Here
$n=11333322$, $k=66$, $T=27141$, and both source and common
agreements are exactly $19955$. The noncanonical upper bound is
$12610$, the integer first-order upper bound is $19802$, and minimum
canonical agreement is $36720$. There are $29028384$ singleton
parameters, with loss ratio $7186/27075>0.2654$. This is an exact
parameter certificate, not an enumeration of the domain.
