...
raw=Path('ramified_contradiction.bin').read_bytes();weights=struct.unpack('<'+'I'*(M*T),raw)
...
result={'PASS':True,'claim':'Original-Jacobian null certificate for ramification-two obstruction','N':N,'K':K,'bank_size':M,'retained_matches':T,'original_equations':M*T,'all_variable_columns':len(coeff),'certificate_nonzero_weights':used,'weighted_coefficients_all_zero':True,'weighted_obstruction':rhs,'seconds':time.monotonic()-start,'sha256':{p:hashlib.sha256(Path(p).read_bytes()).hexdigest() for p in ['fixture.txt','ramified_contradiction.bin','test.cpp','witness.cpp']}}
Path('ramified_jacobian_verified.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2))
