...
nodes=[t+F(u) for t in [a,a*a,a*a*a] for u in range(p)]
...
for support in combinations(range(3*p),p+1):
...
for rec in candidates.values():
 ...
 if n>=9:best.append(rec)
result={'p':p,'modulus':str(F.modulus()),'nodes':list(map(str,nodes)),'tested_six_subsets':tested,'inconsistent':inconsistent,'singular':singular,'candidate_histogram':hist,'nine_match_candidates':best,'seconds':time.time()-start}
Path(__file__).with_suffix('.json').write_text(json.dumps(result,indent=2));print(json.dumps({k:v for k,v in result.items() if k not in ['nodes','nine_match_candidates']},indent=2));print('nine matches',len(best),'noncanonical',sum(not r['canonical'] for r in best))
