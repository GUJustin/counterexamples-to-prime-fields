...
for idx in (21,5,22):
 ...
 for dk in range(-2,3):
  for dn in range(-2,3):
   p=old.copy();p[5]+=dk;p[6]+=dn;m,B,s,U,L,k,n0=p
   if not(0<=k<=s<m and k+1<=n0 and 2*(n0-k-1)<=B):continue
   caps=[(m*O.A-(h if h<n0 else k)*(O.A-131069)+B-1)//131071 for h in range(s+1)]
   if min(caps)<U:
    records.append(dict(dk=dk,dn0=dn,parameters=p.copy(),feasible_L=None,reason='closed rank cutoff cap fails',minimum_cutoff_cap=min(caps)));continue
   gap=O.coeff(O.A,*p)-O.n*O.rank(*p);p[4]+=1;slope=O.coeff(O.A,*p)-O.n*O.rank(*p)-gap;p[4]=L
   rec=dict(dk=dk,dn0=dn,parameters=p.copy(),surplus_at_old_L=gap,surplus_slope=slope)
   if slope<=0:
...
(ROOT/'auxiliary_taylor_order_probe.json').write_text(json.dumps(out,indent=2)+'\n')
for row in out:
 feasible=[r for r in row['records'] if r.get('active_at_contact')];feasible.sort(key=lambda r:r['contact_bound']);print('source',row['source'],'best',feasible[:2]);print('paired +1',next(r for r in row['records'] if r['dk']==r['dn0']==1))
