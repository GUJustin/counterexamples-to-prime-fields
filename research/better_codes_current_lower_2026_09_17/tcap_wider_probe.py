...
rows=[]
for m in range(270,2001,10):
 for ss in sorted(set(m*p//100+d for p in range(29,34) for d in [-1,0,1])):
  z=solve(m,ss,9275)
  if z is not None:rows.append(z)
valid=[x for x in rows if x['min_total_cap'] is not None]
print('cases',len(rows),'fixedcapfeasible',sum(x['required_L'] is not None for x in rows))
print('best',min(valid,key=lambda x:x['min_total_cap']))
(ROOT/'tcap_wider_probe.json').write_text(json.dumps(dict(scope='m270..2000 step10; s=floor(pm/100)+d, p29..33,d=-1,0,1; legitimate one-residue counts only',rows=rows),indent=2)+'\n')
