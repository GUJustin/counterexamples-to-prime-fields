...
A=181284;n=262144;w=131071;delta=A-w+1
lo_r,hi_r=1,35
source=[tuple(map(int,x.split(','))) for x in q.re.findall(r'def source : SourceNumbers := ⟨(.*?)⟩',(q.CACHE/'MovingFiberSourceSound6811.lean').read_text())]
pot=q.POT[:7]
old={key:row['threshold'][:7] for key,row in q.rows.items()}
roots.SOURCES=[x['old'] for x in roots.MANIFEST['sources']]
roots.A=A;roots.GAP=A-w;roots.ERROR_PLUS_ONE=n-A+1
roots.pair_numerator.__defaults__=(A,)
roots.root_lines.cache_clear()
...
for j in range(8):
 ...
 for rr,nums in re.findall(r'def own(\d+) : Array Nat := #\[([^\]]*)\]',text):old_own[j,int(rr)]=[int(x) for x in nums.split(',')]
results=[];checks=0;segment_count=0
for r in range(lo_r,hi_r+1):
 for v in range(160-r):
  y=r+v;finish=9276-y
  ...
  for z in points:
   ...
   checks+=1
  results.append(dict(r=r,v=v,threshold=ts,own=own,stored=[old_own[j,r][v] for j in range(8)],deltas=[own[j]-old_own[j,r][v] for j in range(8)]));segment_count+=len(segments)
 print('r',r,'done',flush=True)
bad=[dict(r=x['r'],v=x['v'],sheet=j,excess=d) for x in results for j,d in enumerate(x['deltas']) if d>0]
out=dict(A=A,contexts=len(results),critical_points=checks,positive_own_deltas=len(bad),worst=sorted(bad,key=lambda x:x['excess'],reverse=True)[:20],rows=results,scope='Old parameters through generic new singleton envelope generator; positive delta measures failure to reproduce stored allowance.')
(q.ROOT/'baseline_singleton_control_generic.json').write_text(json.dumps(out,indent=2)+'\n')
