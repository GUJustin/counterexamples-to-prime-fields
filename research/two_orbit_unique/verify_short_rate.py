...
if __name__=='__main__':
 start=time.monotonic();r=json.loads((BASE/'short_rate_certificate.json').read_text())
 ...
 n=2*m+4;K=2*D-1;assert n==r['n'] and K==r['K']
 ...
 E=K**K*(n-K)**(n-K)
 assert J**5*E>n**(n+5)*2**(5*r['original_prescription_ratio_power_two'])
 assert (100*J)**5*E**2>(w*n)**5*n**(2*n) and 5*(p.bit_length()-1)>n
 out=dict(status='passed',seconds=time.monotonic()-start,n=n,K=K,prime_bits=p.bit_length(),
  ...
  scope='Independent subset-count DP, deterministic primality proof, and exact non-half-rate entropy comparison; no specified domain.')
 (BASE/'short_rate_verification.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out,indent=2))
