...
std::vector<R>r;r.reserve(595665);for(int a=1;a<61;a++)for(int b=a+1;b<62;b++)for(int c=b+1;c<63;c++)for(int d=c+1;d<64;d++)r.push_back({(((v[a]+v[b]+v[c]+v[d])%p)<<6)|U((a+b+c+d)%64),(U(1)<<a)|(U(1)<<b)|(U(1)<<c)|(U(1)<<d)});
...
for(size_t l=0;l<r.size();){size_t h=l+1;while(h<r.size()&&r[h].key==r[l].key)h++;maxfiber=std::max(maxfiber,U(h-l));for(size_t i=l;i<h;i++)for(size_t j=i+1;j<h;j++){pairs++;bool modular=!lifted(r[i].mask,r[j].mask);if(modular)allfresh++;if(!(r[i].mask&r[j].mask)){disjoint++;if(modular){fresh++;if(fresh<=100){if(comma)std::cout<<",";std::cout<<"[";array(r[i].mask);std::cout<<",";array(r[j].mask);std::cout<<"]";comma=true;}}}}l=h;}
