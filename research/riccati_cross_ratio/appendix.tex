...
The cross-ratio identities of Riccati equations, combined with
intersection multiplicities in positive characteristic, give an
ordinary-list bound even when all agreement coordinates are singular.
...
The failure of the $D+2$ count at the boundary is not confined to
small primes. For every prime $p$, the equation
\[
 (X^p-X)P'=-P-P^2,\qquad \deg P\le p-1,
\]
has exactly $p+2=D+3$ solutions over every extension of $\F_p$:
$0$, $-1$, and $(X^p-X)/(X-r)$ for $r\in\F_p$.
Indeed, evaluating at $\F_p$ forces each value of $P$ to be $0$
or $-1$. Let $S$ be the set of coordinates with value $-1$ and
$f=\prod_{r\in S}(X-r)$. Lagrange interpolation gives
$P=(X^p-X)f'/f$. Substitution reduces the equation to $f''=0$.
Since $\deg f\le p$, a nonconstant proper divisor $f$ of $X^p-X$
with $f''=0$ must be linear: its only possible nonzero coefficients
have degrees $0,1,p$. Thus $S$ is empty, a singleton, or all of
$\F_p$, proving the assertion. This family does not prove that
$D+3$ is a universal upper bound at the boundary.
