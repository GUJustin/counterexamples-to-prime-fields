...
int main(int argc,char**argv){
 assert(argc==2);int p=std::stoi(argv[1]);assert(p==41);int k=(p-1)/4;
 ...
 do{
  ...
  for(int v=1;v<p;v++)if(hist[v]>=k-1){int c=power(v,p-2,p);auto P=H;for(int &a:P)a=a*c%p;bank.insert(P);auto Q=P;int scale=power(ns[0],p-2,p);for(int j=0;j<=k;j++)Q[j]=(p-P[j])*power(scale,j,p)%p;Q[0]=(Q[0]+1)%p;bank.insert(Q);}
  count++;int j=k-1;while(j>=0&&s[j]==k+j)j--;if(j<0)break;s[j]++;for(int t=j+1;t<k;t++)s[t]=s[t-1]+1;
 }while(true);
 std::cout<<"{\"p\":"<<p<<",\"length\":"<<p-1<<",\"dimension\":"<<k+1<<",\"known_maximum_agreement\":"<<2*k<<",\"threshold\":19,\"root_subsets\":"<<count<<",\"bank_size\":"<<bank.size()<<",\"complete\":true,\"coefficients\":[";
