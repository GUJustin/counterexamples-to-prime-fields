...
The quadratic-residue indicator gives an elementary obstruction for
the entire nearest-list problem of the full-field Dickson word, not
only for the explicit candidate family used in the lower bound.
It imposes no such restriction on general received words or domains.

\begin{proposition}\label{prop:quadratic-indicator-rigidity}
Let $k\ge1$ and let $F$ contain all $4k$th roots of unity. Suppose
$F$ has characteristic zero or characteristic $p>8k/3$. Put
$w(x)=(1+x^{2k})/2$ on $\mu_{4k}\subset F$.
...
its correct zeros on $x^{2k}=-1$ and correct ones on $x^{2k}=1$. Put
...
After division the identity is pairwise coprime. In positive characteristic all degrees are less
than $p$, since $2k+2h<8k/3<p$. In either characteristic polynomial abc gives
\[
 2k+\alpha+\beta-2t
...
parts of all three polynomials divide this Wronskian. A nonconstant term excludes a zero Wronskian in characteristic zero;
in positive characteristic the degree bound less than $p$ does so.

Thus $E=0$. As $F$ contains a square root of $-1$, we have
...
agreement fraction is $1/2$.

\begin{corollary}\label{cor:dickson-word-first-order-barrier}
Under the hypotheses of Proposition~\ref{prop:quadratic-indicator-rigidity},
and with characteristic different from five, the Dickson received word
$W(x)=(x^k-1)^2/2$ on $\mu_{4k}$ has agreement at most
$\lfloor5k/3\rfloor$ with every degree-$<k$ polynomial, even with
extension coefficients. In particular its maximum agreement fraction
is at most $5/12$, below the quarter-rate first-order threshold.
\end{corollary}
\begin{proof}
Since $W=w-X^k$, a candidate exceeding the cutoff would give a monic
degree-$k$ polynomial $P+X^k$ in Proposition~\ref{prop:quadratic-indicator-rigidity}.
The leading coefficient $c=1/(v-u)$ of each nonconstant listed
polynomial satisfies $4c^4+1=0$, since $(v-u)^4=-4$.
It cannot equal one unless the characteristic is five.
\end{proof}

In particular this applies to every prime-field cyclic domain
$\mu_{4k}\subseteq\F_p^*$ with $p>5$, including the full domain
when $p=4k+1$. Enlarging the candidate bank for this cyclic word cannot
reach the first-order regime. This does not rule out arbitrary
puncturing or changing the word or evaluation domain.
As a finite check, exhaustive enumeration
