...
For every prime $p=4k+1>5$, the Dickson received word
$W(x)=(x^k-1)^2/2$ on $\F_p^*$ has agreement at most
...
It cannot equal one unless $p=5$.
\end{proof}

Consequently enlarging the candidate bank for this fixed full-field
word cannot reach the first-order regime. This does not rule out
