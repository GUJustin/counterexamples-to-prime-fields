...
The later quarter-density puncturing in `../binary_characteristic_transfer/quadratic_frobenius_core/density_quarter_puncturing.tex` removes the source-placement limitation for an extension-field variant. For every prime p>=257 it gives n=(5p²−1)/2, exact endpoint/common A=2p, tested T=isqrt(5p²−2), and (p+1)(p²−1) singleton labels over F_(p⁴), with sources above first order and T below Johnson. Its loss/capacity ratio tends to1−2/sqrt(5), with T the largest integer strictly below Johnson. It remains an extension-alphabet construction on a selected domain of size Theta(p²), not a prime-alphabet or short-domain transfer. The older full-block examples described above retain their original stated parameters.
...
A later exact comparison, [the rate-uniform canonical-bank envelope](KKH_RATE_UNIFORM_CANONICAL_ENVELOPE.md), addresses whether varying the KKH rate can produce the missing superlinear count. For the basic ledger n=sm, strict dimension k=(r−2)m+1 and canonical agreement rm, with arbitrary integers s>=16 and m>=1, placing that agreement above the full first-order curve forces the canonical bank to have fewer than 4n members. If s and m are powers of two, the bound is fewer than 2n. The comparison also applies when the code is enlarged or the tested threshold is lowered. It bounds only labels supplied by this canonical bank, not every nearby label on the line, and does not cover a new construction with additional witnesses. The strict +1 in message dimension is essential at the high-rate endpoint.
