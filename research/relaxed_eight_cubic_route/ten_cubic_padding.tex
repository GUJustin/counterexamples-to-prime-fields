\subsection{Simultaneous padding of the rational cubic seed}\label{subsec:simultaneous-padding}

\begin{theorem}\label{thm:ten-cubic-padding}
...
\paragraph{The rational source and its five-match cubics.}
Use the sixteen-node rational source and eight cubics $P_0,\ldots,P_7$ of Theorem~\ref{thm:eight-cubic-smooth-seed}, in its affine coordinate $U=1/(X-3)$.
The complete set of additional cubics with at least five source matches consists of the following eight $Q_i$:
\begin{table}[ht]
\centering
...
\end{array}$
\caption{The eight additional cubics $Q_i(U)=\sum_{j=0}^3q_{ij}U^j$ with five matches on the rational source.}\label{tab:fresh-five-cubics}
\end{table}
All eight $Q_i$ have exactly five matches. These finite completeness statements can be checked without solving any nonlinear equations: for each of the $\binom{16}{4}=1820$ four-node subsets interpolate its unique cubic, deduplicate the coefficient vectors, and count its matches with the rational source table. Exact rational arithmetic gives
\[
