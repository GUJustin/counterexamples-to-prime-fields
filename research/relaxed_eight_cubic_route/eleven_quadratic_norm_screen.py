...
P=Path(__file__).parent;p=101;start=time.monotonic();d=json.loads((P/'rational_seed.json').read_text());fr=json.loads((P/'pair_padding_gate.json').read_text())['candidates']
def red(s):
 q=Fraction(s);return q.numerator*pow(q.denominator,-1,p)%p
def ev(c,x):return sum(a*pow(x,j,p) for j,a in enumerate(c))%p
for candidate_p in [p for p in range(19,300) if all(p%j for j in range(2,int(p**.5)+1))]:
 p=candidate_p
 try:
  old=list(map(red,d['affine_nodes']));oldw=list(map(red,d['affine_word']));r=red('-80/163');ss=red('-135/262');g=[red('4525/15304'),red('4175/3826'),1];roots=[x for x in range(p) if ev(g,x)==0]
  if len(roots)!=2 or len(set(old+[r,ss]+roots))!=20:continue
  Q=list(map(red,fr[0]['coefficients']));Q2=list(map(red,fr[2]['coefficients']));X=old+[r,ss,roots[0]];W=oldw+[ev(Q,r),ev(Q,ss),ev(Q2,roots[0])];break
 except ValueError:continue
else:raise RuntimeError('no good prime')
assert len(set(X))==19
...
for size in [12,11]:
 for sub in itertools.combinations(range(19),size):
...
(P/'eleven_quadratic_norm_screen.json').write_text(json.dumps(out,indent=2));print({k:(len(v) if isinstance(v,list) else v) for k,v in out.items()})
