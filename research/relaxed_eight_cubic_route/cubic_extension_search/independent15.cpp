 ...
 for(int half=0;half<2;half++)for(unsigned mask=0;mask<(1u<<24);mask++){
  ...
  for(int ref=0;ref<34;ref++){
   ...
   for(int j=0;j<40;j++)if(j!=ref){
    ...
    int actual=0;for(int i=0;i<48;i++)actual+=eval(c,X[i])==W[i];if(actual<15)abort();hits.insert(c);
   }
  }
 }
 cout<<"{\"subsets_visited\":"<<visited<<",\"planes\":"<<pencils<<",\"hits\":[";bool first=true;for(auto c:hits){if(!first)cout<<",";first=false;cout<<"[";for(int j=0;j<10;j++){if(j)cout<<",";cout<<c[j];}cout<<"]";}cout<<"]}\n";
}
