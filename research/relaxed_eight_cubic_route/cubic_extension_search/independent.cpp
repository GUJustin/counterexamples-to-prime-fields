...
int main(){
 ...
 for(int half=0;half<2;half++)for(unsigned mask=0;mask<(1u<<24);mask++){
  if(__builtin_popcount(mask)!=9)continue;visited++;
  ...
  for(int k=1;k<9;k++)for(int i=8;i>=k;i--)dd[i]=mul(sub(dd[i],dd[i-1]),diffInv[idx[i]][idx[i-k]]);
  vector<int> q{dd[8]};for(int k=7;k>=0;k--){vector<int>b(q.size()+1);for(int j=0;j<(int)q.size();j++){b[j]=sub(b[j],mul(X[idx[k]],q[j]));b[j+1]=add(b[j+1],q[j]);}b[0]=add(b[0],dd[k]);q=b;}
  ...
  array<int,289>count{};for(int i=0;i<48;i++){if(i/24==half&&(mask&(1u<<(i%24))))continue;int denominator=eval(loc,X[i]);if(!denominator)abort();int lambda=mul(sub(W[i],eval(q,X[i])),inverse[denominator]);count[lambda]++;}
  for(int lambda=0;lambda<289;lambda++)if(count[lambda]>=8){vector<int> c(10);bool down=true;for(int k=0;k<10;k++){c[k]=add(k<9?q[k]:0,mul(lambda,loc[k]));if(k%3&&c[k])down=false;}if(down){descended++;continue;}int actual=0;for(int i=0;i<48;i++)actual+=eval(c,X[i])==W[i];if(actual!=9+count[lambda])abort();hits.insert(c);}
 }
