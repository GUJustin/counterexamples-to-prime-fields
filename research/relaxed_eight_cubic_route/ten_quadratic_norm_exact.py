...
import sympy as S
P=Path(__file__).parent;start=time.monotonic();d=json.loads((P/'rational_seed.json').read_text());fr=json.loads((P/'pair_padding_gate.json').read_text())['candidates'];screen=json.loads((P/'ten_quadratic_norm_screen.json').read_text());assert not screen['unresolved_systems']
z=(Q(0),Q(0));one=(Q(1),Q(0));t0=Q(25,358);t1=Q(-125,358)
...
def ev(c,x):
 ...
 return r
X=[(Q(a),Q(0)) for a in d['affine_nodes']]+[(Q(0),Q(1)),(t1,Q(-1))];W=[(Q(a),Q(0)) for a in d['affine_word']];F=[(Q(a),Q(0)) for a in fr[0]['coefficients']];W.extend(ev(F,x) for x in X[16:])
...
x=S.Symbol('x');theta=(-125+55*S.sqrt(17))/716;records=[];hits=[]
...
for key,origin in unique.items():
 ...
 poly=S.Poly(sum(sx(a)*x**i for i,a in enumerate(J)),x,extension=S.sqrt(17));odd=0 if poly.is_zero else sum(f.degree() for f,e in poly.sqf_list()[1] if e%2)+(6-poly.degree())%2
 nh=[j for j in range(18) if add(add(mul(W[j],W[j]),mul(ev(v[:4],X[j]),W[j])),ev(v[4:],X[j]))==z]
