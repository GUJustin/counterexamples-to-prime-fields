...
## Growing-characteristic transfer checkpoint

A separately audited research theorem now uses one random padding set inside the full additive domain. At every fixed rate, the existing locator compiler yields all native challenges at length N=p^8 with common-agreement gap Omega_rho(sqrt(N)); the archived outside-seed consequence required characteristic N^(1/10) for this qualitative combination, whereas the new one permits N^(1/8). At N=p^4 it gives more than half the native labels, and at N=p^5 a fraction 1−O(1/p), with gaps of orders sqrt(N) and N^(3/5), respectively. The exact theorem, finite sufficient conditions, field distinctions, and independent audit are in ../binary_characteristic_transfer/GROWING_CHARACTERISTIC_FULL_DOMAIN_PADDING.md and its _AUDIT.md companion.

This is a stronger growing-characteristic specialization of the existing binary-paper compiler, not a new compiler. It is retained as a research note pending the geometric label-coverage investigation rather than automatically adding another manuscript appendix. The native all-label statement has only the direction individually far; a separate larger-extension statement makes both sources far. The characteristic remains much smaller than the message degree, and the normalized agreement gap vanishes. It therefore gives neither prime-alphabet DKT tightness nor a better.codes improvement.

The d=4,s=2 canonical locator image has now been independently proved to contain p^4/2+O(p^(7/2)) labels uniformly for every nonzero head parameter. The proof gives an exact binary-quadratic split criterion and handles the norm-one exceptional fibers. An independent eight-variable integer-polynomial replay passed. This closes the special-parameter route to almost-all canonical labels in that family; it does not bound additional witnesses after padding. The positive follow-up is d=5,s=2, whose trace-dual Plucker formula gives five hyperplanes on Gr(2,5). Bounded theta=1 checks give full coverage for p=3,7 and one missing scalar label for p=5,11. These checks are exploratory, not an admitted infinite-family theorem.

## Admitted five-dimensional strengthening

**Capacity-margin priority correction.** This strengthening concerns native label coverage and characteristic, not distance from capacity. Its normalized capacity margin and guaranteed common-agreement loss are both of order N^(-2/5). At fixed rate this is asymptotically smaller than the inverse-logarithmic margin in KKH Theorem 1. It therefore does not improve the known capacity-margin counterexamples. Do not compare it against (log N)/N as a substitute for the existing inverse-logarithmic benchmark. Almost-complete native coverage is a different feature and does not establish a better.codes improvement or practical tightness. Further work should prioritize a substantially larger margin on short domains over refinements of this asymptotic family.

Theorem N.28 and Lemma N.29 now prove the stronger transfer in the manuscript: at every fixed rate and sufficiently large p, N=p^5 has at least N−3 native bad challenges and common-agreement gap floor((1−rho)p³/2); every challenge is bad on a density1/3 prime sequence. The exact canonical image is Fp5 minus the scalar roots of H=Z³+2Z²+3Z+1. The proof combines the original locator compiler, internal padding, published Montanucci–Zanella off-scalar exclusion, and an independently checked scalar Pfaffian calculation. It uses only individually replayed elementary arguments from Lu's recent preprint, not the full new classification. Three independent receipts check original locators, parameter mapping and the scalar geometry; symbolic identities and finite cases pass. The full-page one-inch-margin PDF has261pages and the new pages render cleanly.

At the actual M31 and BabyBear primes, exact polynomial factorization gives exactly one canonical excluded label, hence at least p^5−1 bad native challenges in each quintic extension. The domain length is approximately2^155. This is field compatibility, not practical-domain compatibility or a protocol security ceiling. The native fixed-rate first source is always near because H(0)=1; only the direction is individually far. A separate unpadded/punctured two-far-source strengthening is being audited and is not part of this publication checkpoint.
