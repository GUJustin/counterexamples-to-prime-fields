...
int main(int argc,char**argv){
 ...
 uint64_t count=0;int observed=0; std::set<std::array<int,40>> bank;
 for(int mask:masks){
  ...
  do{
   ...
   for(int j=0;j<30;j++){
    ...
    if(a+remaining<15)break;
   }
   count++;observed=std::max(observed,a);
   if(a>=15){
    std::array<int,40> v;int truea=0;
    for(int j=0;j<40;j++){v[j]=(val+mul(Z[j],eval(x[j])))%41;truea+=v[j]==w[j];}
    assert(truea==a);
    for(int t=0;t<10;t++){std::array<int,40> shifted;for(int j=0;j<40;j++)shifted[j]=v[(j+4*t)%40];bank.insert(shifted);}
   }
   int j=5;while(j>=0&&s[j]==24+j)j--;if(j<0)break;s[j]++;for(int h=j+1;h<6;h++)s[h]=s[h-1]+1;
  }while(true);
 }
 assert(count==22ULL*593775);
 std::cout<<"{\"p\":41,\"coset\":"<<coset<<",\"threshold\":15,\"anchor_orbits\":22,\"supports_checked\":"<<count<<",\"primitive_root\":"<<g<<",\"complete\":true,\"bank_size\":"<<bank.size()<<",\"values\":[";
 bool first=true;for(const auto&v:bank){std::cout<<(first?"":",")<<"[";first=false;for(int j=0;j<40;j++)std::cout<<(j?",":"")<<v[j];std::cout<<"]";}std::cout<<"]}\n";
}
