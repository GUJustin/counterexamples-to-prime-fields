...
int main(int argc,char**argv){assert(argc==6);I n=262144,w=131071;int m=stoi(argv[2]),Q=stoi(argv[4]),S=stoi(argv[5]);I D=181275LL*m,L=stoll(argv[3]);assert(D>(m-1)+(w-1)*Q);node();node();vector<vector<int>> ids(Q+1,vector<int>(S+1,-1));vector<pair<int,int>>vars;I B=0;for(int j=0;j<=S;j++)for(int i=0;i+j<=Q;i++){int id=node();ids[i][j]=id;vars.push_back({i,j});I c=(L-i-j+1)*(D-w*i-(w-1)*j);assert(c>0);B+=c;edge(0,id,c);}for(auto [i,j]:vars)if(i)edge(ids[i][j],ids[i-1][j],B+1);
for(int q=0;q<=Q;q++)for(int ell=0;ell<m;ell++){vector<int>a;for(int i=max(0,q-S);i<=min(q,ell);i++)a.push_back(ids[i][q-i]);if(a.empty())continue;int z=node();I K=n*(L-q+1);for(int x:a)edge(x,z,K);edge(z,1,(m-ell)*K);}
