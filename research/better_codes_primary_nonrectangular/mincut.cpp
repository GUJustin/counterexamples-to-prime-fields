...
int main(int argc,char**argv){assert(argc==2);I n=262144,w=131071,D=20846625,L=274277;int m=115,Q=159,S=35;node();node();int ids[160][36];fill(&ids[0][0],&ids[0][0]+160*36,-1);vector<pair<int,int>>vars;I B=0;for(int j=0;j<=S;j++)for(int i=0;i+j<=Q;i++){int id=node();ids[i][j]=id;vars.push_back({i,j});I c=(L-i-j+1)*(D-w*i-(w-1)*j);B+=c;edge(0,id,c);}for(auto [i,j]:vars)if(i)edge(ids[i][j],ids[i-1][j],B+1);
