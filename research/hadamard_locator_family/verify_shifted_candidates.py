...
out={}
for p in (29,43):
 d=json.loads((P/f'shifted_p{p}.json').read_text());hist={};examples=[]
 for row in d['positive_gcds']:
  ...
  h=row['h'];base={(0,1):1,(0,2):b,(0,3):c,(1,2):q*inv(c)%p,(1,3):q*inv(b)%p,(2,3):q};edge={ij:(e+h)%p for ij,e in base.items()};x=lambda i,j:edge[tuple(sorted((i,j)))];beta=[0,(q-b*c)%p,(q-c)%p,(q-b)%p]
  ...
  Mh=q*M+h*W;Nh=N+h*J;t=-Nh*inv(Mh)%p
  a=[1]+[(1+t*beta[i]*(sum(beta)-2*beta[i]))%p for i in range(1,4)]
  pp=-(2*h*N+(h*h+q)*J)*inv(Nh)%p;k=(q-b*b)*(q-c*c)*(q-1)*inv(Mh)%p
  ...
  for (i,j),ee in edge.items(): assert B[i](ee)==B[j](ee)
  ...
  collision=K.gcd(K.derivative()).degree();assert K.degree()==28 and collision>0;hist[collision]=hist.get(collision,0)+1
...
(P/'verify_shifted_candidates.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2))
