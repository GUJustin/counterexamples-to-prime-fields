...
constant number of agreement coordinates of improvement. For historical comparison, optimizing monomial supports or
translation-stable nonmonomial jet spaces cannot improve the cubic
list-budget and fifth-power MCA-budget dependence in the September 10
draft's specified reconstruction formulas. Separately, for the current
ePrint's full-coefficient monomial supports downward in $Y_0$, with either
graded row or column-only test, we prove necessary
quadratic list-budget and fourth-power regular-MCA-budget dependence.
These finite-length cost statements use an explicit corrected margin,
uniformly in multiplicity. A grouped deletion-and-sorting argument transfers
positive graded feasibility to a coordinatewise order ideal. The row-test
conclusion also extends to total-degree-graded translation-stable nonmonomial
jet spaces with the same full coefficient cutoff. An explicit high-rate support optimization gives the sharp continuum
threshold above half rate, improving a known sufficient curve at every
rate above $8-3\sqrt6$. Two exact
finite certificates instantiate this improvement.
...
\noindent\textbf{Version distinction.} The current 145-page ePrint
2026/2056~\cite{DKTcurrent} proves first-order list and MCA bounds
$O_\rho(n/\eta_1^2)$ and $O_\rho(n^2/\eta_1^4)$, respectively.
Section~\ref{sec:gap-costs}'s cubic/fifth-power budget converse concerns
only the explicitly stated older reconstruction formulas. It does not
establish tightness of these current bounds. Section~\ref{sec:current-graded}
separately proves the current powers necessary for monomial supports
downward in $Y_0$ with full derivative-weighted coefficient prefixes,
using either the graded row or column-only test. The result uses this note's stronger local-rank
characteristic hypothesis. This is necessity for the declared numerical
ledger, not intrinsic tightness. The current ePrint also
acknowledges the high-rate refinement audited in Section~\ref{sec:high-rate}.
...
certificates, a subsequent uniform finite-length extension, and a
converse for the inverse-margin costs of the older bounds, and the
restricted current graded-cost converse. No independent referee or formal verification
...
The exact verifiers check 320 local matrices, 3750 rearrangement and
marginal comparisons, and 3128 one-sided column inequalities by rational
polygon integration. Independent diagonal sums reproduce the finite
certificates after 21295 direct checks of the summation formula.
Further checks cover 1920 finite-cutoff ranks, 32752 half-rank instances,
3743 arbitrary-support rank estimates, the narrow-cap obstruction,
and three positive-surplus parameter certificates. Nonmonomial
degeneration passes 504 rank comparisons, including 196 strict drops.
The finite-length cost transfer passes 6750 reductions, including 1054
positive-surplus ratio comparisons and a near-critical certificate at
length one million. The high-rate shape passes 220 exact polygon
integrations and 4851 endpoint comparisons. These scripts and reports
are in \path{research/first_order_support_audit/}.

The current graded-cost adaptation additionally passes 79002 algebraic
reconstruction-degree checks and 16681 low-degree rank and negative-area
checks for multiplicities 16 through 512, recorded in
\path{research/current_graded_method_costs/}.
The computations corroborate formulas; the proofs establish the
...
\bibitem{DKTcurrent} Q. Dao, S. D. Kominers, and J. Thaler.
...
145-page version retrieved September 17, 2026.
