...
def construct(source="Singer",mixed=False):
 ...
 if CENSUS_SOURCE is not None:D=list(CENSUS_SOURCE)
 S=sorted(set(range(q))-{(-i)%q for i in D})
 assert len(S)==64
 if not PROFILE_MODE:assert all(sum((a-b)%q==t for a in D for b in D)==31 for t in range(1,q))
 progress("source polynomial")
 B=list(reversed([convert(v) for v in rootpoly(S)]))
 targets=[]
 requested=[(O[0],sorted(O[0]*j%q for j in D)) for O in orbits] if CENSUS_TARGETS is None else CENSUS_TARGETS
 for name,T in requested:
  progress("target "+str(name))
  assert len(T)==r and len(set(T))==r
  assert {2*j%q for j in T}==set(T)
  if PROFILE_MODE:
   assert all(sum((a-b)%q==t for a in D for b in D)+sum((a-b)%q==t for a in T for b in T)==62 for t in range(1,q))
  targets.append((name,T,list(reversed([convert(v) for v in rootpoly(T)]))))
 assert len({tuple(a) for _,_,a in targets})==len(targets)
 negD=sorted((-j)%q for j in D)
 Aneg=list(reversed([convert(v) for v in rootpoly(negD)]))
...
if __name__=='__main__':
 ap=argparse.ArgumentParser();ap.add_argument('--h',default='95');ap.add_argument('--output',required=True);ap.add_argument('--mixed',action='store_true');ap.add_argument('--source',default='Singer');ap.add_argument('--targets');ap.add_argument('--resume',action='store_true');ap.add_argument('--census');ap.add_argument('--source-class',type=int,default=0);ap.add_argument('--target-indices');ap.add_argument('--target-start',type=int,default=0);ap.add_argument('--target-stop',type=int,default=80);ap.add_argument('--profiles');ap.add_argument('--profile-class',type=int,default=0);ap.add_argument('--relative-targets',default='0');a=ap.parse_args()
 if a.census:
  row=next(x for x in json.load(open(a.census)) if x['q']==127)
  CENSUS_SOURCE=row['unit_equivalence_classes'][a.source_class]['representative']
  ii=list(range(a.target_start,a.target_stop)) if a.target_indices is None else [int(x) for x in a.target_indices.split(',')]
  CENSUS_TARGETS=[('idx:'+str(i),row['normalized_difference_sets'][i]) for i in ii]
  a.source='census:'+str(a.source_class)
 if a.profiles:
  data=json.load(open(a.profiles));assert data['q']==127
  row=data['unit_classes'][a.profile_class]
  CENSUS_SOURCE=sorted((-x)%127 for x in row['C'])
  assert set(row['S'])==set(range(127))-set(row['C'])
  ii=[int(x) for x in a.relative_targets.split(',')]
  CENSUS_TARGETS=[]
  for i in ii:
   mask=row['compatible_T_orbit_masks'][i]
   T=sorted(x for j,O in enumerate(data['doubling_orbits']) if (mask>>j)&1 for x in O)
   CENSUS_TARGETS.append(('relative:'+str(i),T))
  PROFILE_MODE=True;a.source='profile:'+str(a.profile_class)
