...
def kernel(M):
 ...
 for j in free:
  ...
  out.append(v)
 return out
pole=K.from_sympy(s.Rational(15,26)-3*s.sqrt(3)/13+s.I*(s.Rational(5,13)+9*s.sqrt(3)/26))
start=time.time();hist={};hits=[];rows=[]
for h in range(4):
 yes=[j for j in range(12) if w[j]==Hs[h][j]]
 no=[j for j in range(12) if w[j]!=Hs[h][j]]
 for r,t,vv in itertools.combinations(yes,3):
  zs=[(xs[j]-xs[r])*(xs[j]-xs[t])*(xs[j]-xs[vv])/((xs[j]-pole)*(w[j]-Hs[h][j])) for j in no]
  M=[[K.one,q,q*q,-xs[j],-xs[j]*q,-xs[j]*q*q] for q,j in zip(zs,no)]
  ker=kernel(M);hist[len(ker)]=hist.get(len(ker),0)+1
  row={'old_index':h,'root_indices':[r,t,vv],'kernel_dimension':len(ker)}
  if len(ker)==1:
   v=ker[0];disc=v[4]*v[4]-K.convert(4)*v[3]*v[5]
   row['denominator_discriminant_zero']=not bool(disc)
   row['kernel']=[str(K.to_sympy(x)) for x in v]
   row['denominator_discriminant']=str(K.to_sympy(disc))
   row['undefined_node_indices']=[j for q,j in zip(zs,no) if not(v[3]+v[4]*q+v[5]*q*q)]
   row['max_repeated_z']=max(sum(qq==q for qq in zs) for q in zs)
   if not disc:
