...
for inds in combinations(range(4),2):
 ...
 for a,b in [(a,b) for a in even for b in odd]+list(combinations(odd,2)):
  if True:
   g=eqs[a].gcd(eqs[b])
   rawdegree=g.degree()
   guard=F8*F12*F6*f
   while g.degree()>0:
    ...
    g=g.exquo(gg)
   if g.degree()>0:hits.append(dict(cosets=[a,b],gcd=str(g.as_expr()),degree=g.degree(),raw_degree=rawdegree))
 outputs.append(dict(special_orbits=inds,stabilizer=stabilizer,candidate=str(f.as_expr()),hits=hits))
out=dict(field='Q(i,sqrt(2))',group_order=24,tests=108,target_agreement=15,seconds=time.monotonic()-start,candidates=outputs);(P/'pilot15.json').write_text(json.dumps(out,indent=2));print([(a['special_orbits'],a['stabilizer'],[(h['degree']) for h in a['hits']]) for a in outputs])
