"""Independent stdlib F29^3 replay of EVERY source/Gauss-image witness."""
import json,hashlib,time
from pathlib import Path
P=Path(__file__).parent;raw=(P/'gauss_samples.json').read_bytes();d=json.loads(raw);gr=(P/'gauss_determinant.json').read_bytes();br=(P.parent/'gate.json').read_bytes()
assert d['gauss_input_sha256']==hashlib.sha256(gr).hexdigest() and d['gate_input_sha256']==hashlib.sha256(br).hexdigest()
assert d['field_modulus']==[28,28,0,1]
zero=(0,0,0);one=(1,0,0)
def add(a,b):return tuple((x+y)%29 for x,y in zip(a,b))
def sub(a,b):return tuple((x-y)%29 for x,y in zip(a,b))
def mul(a,b):
 a0,a1,a2=a;b0,b1,b2=b
 d3=a1*b2+a2*b1;d4=a2*b2
 return((a0*b0+d3)%29,(a0*b1+a1*b0+d3+d4)%29,(a0*b2+a1*b1+a2*b0+d4)%29)
def powers(a,n):
 r=[one]
...
def evaluate(terms,xp,yp):
 ...
 return(a%29,b%29,c%29)
def cross(a,b):return(sub(mul(a[1],b[2]),mul(a[2],b[1])),sub(mul(a[2],b[0]),mul(a[0],b[2])),sub(mul(a[0],b[1]),mul(a[1],b[0])))
def parse(v):
 assert len(v)==3 and all(type(x)==int and 0<=x<29 for x in v)
 return tuple(v)
G=json.loads(gr)['factors'][0]['terms'];B=next(x for x in json.loads(br) if x['bank']=='paley')
F=[[(i,j,c) for (i,j),c in zip(B['columns'],r) if c] for r in B['kernel']]
FX=[[(i-1,j,c*i%29) for i,j,c in T if i] for T in F];FY=[[(i,j-1,c*j%29) for i,j,c in T if j] for T in F]
...
for point,image in zip(d['source_points'],d['image_points']):
 ...
 xp,yp=powers(x,73),powers(y,21);assert evaluate(G,xp,yp)==zero
...
out={'status':'PASS','sample_sha256':hashlib.sha256(raw).hexdigest(),'field_arithmetic':'independent tuple arithmetic, r^3=r+1',
