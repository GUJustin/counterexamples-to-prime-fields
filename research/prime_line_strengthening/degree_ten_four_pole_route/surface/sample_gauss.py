"""Exact Gauss-image point certificates over F29^3; bounded deterministic sampling.
...
P=Path(__file__).parent;raw=(P/'gauss_determinant.json').read_bytes();gdata=json.loads(raw);assert len(gdata['factors'])==1 and gdata['factors'][0]['exponent']==1
G=[tuple(map(int,t)) for t in gdata['factors'][0]['terms']]
rawb=(P.parent/'gate.json').read_bytes();B=next(v for v in json.loads(rawb) if v['bank']=='paley');forms=[[(int(i),int(j),int(c)) for (i,j),c in zip(B['columns'],row) if c] for row in B['kernel']]
modulus=[28,28,0,1];F=fq_default_ctx(modulus=fmpz_mod_poly_ctx(29)(modulus),var='r');PX=fq_default_poly_ctx(F);zero=F(0);one=F(1);zeta=F(16)
assert zeta**7==one and zeta!=one
assert all((i+5*j-3)%7==0 for i,j,c in G)
for char,terms in zip((1,3,5),forms):assert all((i+5*j-char)%7==0 for i,j,c in terms)
def enc(a):return list(map(int,a.to_list()))
def code(a):return sum(v*(29**i) for i,v in enumerate(enc(a)))
def element(k):return F([k%29,(k//29)%29,k//841])
def powers(a,n):
 out=[one]
...
# F29-irreducible residual factor.
xp,yp=powers(F(1),73),powers(F(10),21)
assert evaluate(G,xp,yp)==zero and evaluate(G,xp,yp,1,0)==F(27) and evaluate(G,xp,yp,0,1)==F(14)
out={'schema_version':1,'p':29,'field_modulus':modulus,'coordinate_encoding':'[c0,c1,c2] in basis1,r,r^2',
 ...
 'source_points':[],'image_points':[],'smooth_rational_point':[1,10],'smooth_gradient':[27,14],
...
checkpoint()
for trial in range(29**3):
 if calls>=args.max_x or len(images)>=args.target or time.monotonic()-start>args.seconds:break
 x=element((997*trial+17)%(29**3))
 if code(x) in seen_x:continue
 # Mark the complete multiplicative/Frobenius X orbit before root finding.
 for q in range(3):
  for j in range(7):seen_x.add(code((x**(29**q))*(zeta**j)))
 xp=powers(x,73);coeff=[sum((c*xp[i] for i,j,c in G if j==d),zero) for d in range(22)];poly=PX(coeff)
