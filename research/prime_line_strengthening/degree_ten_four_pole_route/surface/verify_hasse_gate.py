...
for row in g['affine_certificate']:
 ...
 for (a,b,c),v in h:
  if b>=r and c>=s:
   ...
   out[key]=(out[key]+row['coefficient']*v*math.comb(b,r)*math.comb(c,s))%29
 ...
 for (a,b,c),v in h:
  if a==r and b>=s:
   for j,m in enumerate(row['multiplier_coefficients']):
    out[b-s+j]=(out[b-s+j]+m*v*math.comb(b,s))%29
assert {k:v for k,v in out.items() if v}=={0:1}
# At [0:1:0], dehomogenize b=1. Distinct (a,c) remain distinct.
endpoint=min(a+c for (a,b,c),v in h if v%29)
assert endpoint==6 and endpoint<15
