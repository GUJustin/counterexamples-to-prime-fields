...
for case in receipt['cases']:
    ...
    for row in selected:
        ...
        matrix.append([(math.comb(i,dx)*math.comb(j,dy)*x**(i-dx)*y**(j-dy))%29
                       if i>=dx and j>=dy else 0 for i,j in monomials])
    det=determinant_integer(matrix)%29
    assert det==case['minor_determinant'] and det!=0
    results.append({'t':t,'rows':len(labels),'columns':len(monomials),'minor_mod29':det})
