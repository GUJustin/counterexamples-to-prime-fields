...
basis=[];basis_meta=[];pending=[];pending_meta=[];count=0;start=time.time();log=[]
def reduce_batch():
 global basis,basis_meta,pending,pending_meta
 rows=basis+pending;meta=basis_meta+pending_meta
 M=nmod_mat(rows,p);T=M.transpose();del M
 R,rank=T.rref();del T
 selected=[next(j for j in range(len(rows)) if int(R[i,j])) for i in range(rank)]
 del R
 basis=[rows[j] for j in selected];basis_meta=[meta[j] for j in selected];pending=[];pending_meta=[]
 log.append([count,rank,time.time()-start]);print('rows',count,'rank',rank,'seconds',time.time()-start,flush=True);gc.collect()
for t in range(15):
 ...
 for db in range(t+1):
  ...
  for mb in range(t+1):
   ...
   for mc in range(t+1-mb):
    ...
    pending.append(row);pending_meta.append([db,dc,mb,mc]);count+=1
    if len(pending)>=320:
     reduce_batch()
     if len(basis)==len(cols):break
if pending:reduce_batch()
