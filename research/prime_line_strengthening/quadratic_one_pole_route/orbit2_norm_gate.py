...
P=Path(__file__).parent;D=json.loads((P/'orbit2_bank83.json').read_text());p=83;xs=D['base'];ys=D['word']
def interp(F,x):
 val=0
 for j in F:
  num=den=1
...
(P/'orbit2_norm_gate.json').write_text(json.dumps(out,indent=2));print({f:{r:sum(len(x['full'])==f and x['rank']==r for x in out) for r in set(x['rank'] for x in out if len(x['full'])==f)} for f in range(4)});print('nontrivial kernels',[(x['full'],x['empty'],x['kernel']) for x in out if x['kernel']])
