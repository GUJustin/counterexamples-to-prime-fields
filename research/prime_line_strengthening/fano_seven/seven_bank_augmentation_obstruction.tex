...
The files are in \path{quadratic_one_pole_route} and
\path{cubic_cover_norm_gate}, with independent verification
receipts and the source bank arrays.  The orbit-2 elimination and
Jacobian are in \path{fano_seven/orbit2_rigidity_audit.json};
its branch formulas are replayed from
\path{nonfano2_projective.json} and the two branch files.
