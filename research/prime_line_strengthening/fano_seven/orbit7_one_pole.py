...
r=Path(__file__).parent;s=runpy.run_path(str(r/'orbit7_independent_field_audit.py'));K=s['K'];xs=s['tnodes'];ys=s['word'];start=time.monotonic()
...
out={'complete':True,'supports':supports,'bases':bases,'polynomial_bases':polynomial_bases,'hits':hits,'quartic_at_infinity_supports':infinity_hits,'seconds':time.monotonic()-start,'scope':'Exact cubic-number-field coefficients; proper finite pole off the 14-node affine domain, numerator degree at most four.'}
(r/'orbit7_one_pole.json').write_text(json.dumps(out,indent=2)+'\n');print(json.dumps(out),flush=True)
