...
D=json.loads((P.parent/'quadratic_one_pole_route/fiber_patterns.json').read_text());p=29;xs=D['base'];ys=D['word']
def reduce(M):
...
(P/'gate.json').write_text(json.dumps(out,indent=2));print([(a['full'],len(a['matrix']),len(a['columns']),a['rank'],len(a['kernel'])) for a in out])
