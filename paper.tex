...
and no correlated agreement. A separate lift preserves a selected-list
obstruction at a fixed positive gap and arbitrarily large lengths.
...
A selection argument then shows that the coefficient of $n$ in a
line-decodability threshold must account for this list size. If the
coefficient is smaller, no additive term that is $o(n)$ along the
resulting family can repair it. This second construction
\emph{does} have correlated agreement. It rules out a sublinear
remainder along this fixed-gap family.
...
controlling that dependence. The fixed-gap lift in
Theorem~\ref{fg:fixed-gap} rules out a prescribed numerical
coefficient with a remainder sublinear along its increasing-length
family. Its field grows, and its chosen list size remains fixed.
...
There are two qualifications. First, the fixed-gap construction has
correlated agreement. It refutes the quantitative selected-witness
line threshold; it is not a fixed-gap example of global failure of
correlated agreement. Second, it does not refute a threshold expressed
using the \emph{actual} maximum list size $\ell_C(\theta)$. The
constructed selected count is $Ln\le\ell_C(\theta)n$. Replacing a
false numerical estimate for list size and retaining a theorem that
uses actual list size are different operations.
...
the fixed-gap lift retains a fixed list rather than an exponentially
growing one.
