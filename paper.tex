...
nearby quadratics, while a finite DKT certificate bounds every word's
list by $22N$. Thus the uniform list bound has the correct linear
dependence on $N$ along this parameter family. The threshold stays
...
$\sqrt3<c<2$: for $p\ge401$, the same word has $N/2$ witnesses
and the uniform upper bound is $137N$. Thus linear list growth is
...
for every fixed $\sqrt2<c<2$, using the same finite support machinery
with larger fixed derivative caps. The additional range below $\sqrt3$
