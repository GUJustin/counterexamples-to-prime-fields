...
A second construction retains the two-far, all-interior-close profile
strictly below Elias with a polynomial-size prime alphabet:
$p=\Theta((n/\log n)^\beta)$ for any fixed $\beta>12/5$.
Its distance below capacity is $\eta=\Theta(1/\log n)$, and both
endpoints are farther by $(1/2+o(1))\eta$.
The domain is a selected union of multiplicative cosets.
...
At vanishing rate, we give a prime-field line with more than
$n^{3/2}/4$ exceptional challenges for the code of quadratic polynomials.
Both inputs are outside the tested radius, and every qualifying list is
a singleton, at an error radius beyond Johnson and covered by the
Dao--Kominers--Thaler upper bounds. The common agreement misses that
threshold by one coordinate.
A collision-averaging variant makes a constant fraction of all prime-field
challenges exceptional with singleton lists.
Randomized padding further gives a source separation of order
$\log n/\log\log n$, while a constant fraction of prime-field challenges
have singleton lists. The message dimension remains three; the rate and
fractional source separation vanish. Allowing dimension $\Theta(n^\alpha)$
gives separation $\Theta(n^\alpha)$ and $\Theta(n^{3(1-\alpha)/2})$
singleton exceptions for every fixed $0<\alpha\le1/3$. These examples
attain a separate upper bound for their conic family of witnesses up to
a constant factor, but do not establish tightness of the general bounds.
Over extension fields, a further construction retains $\Theta(n^{3/2})$
exceptions and a $\Theta(\sqrt n)$ agreement gap while raising the
characteristic to $\Theta(\sqrt n)$. A separate affine-witness
construction asymptotically attains a universal triple-count bound.
For ordinary common agreement alone, a cubic-extension construction
strengthens the exceptional probability to one: every affine mixture is
nearby, although no pair of quadratic codewords explains the inputs on
the required common set. Almost every mixture has a singleton list.
Here the inputs themselves are nearby; the common-agreement deficit,
of order $\sqrt n$, is the obstruction.
Puncturing at most $p$ coordinates then makes $p$ inputs individually
far. Over $\F_{p^3}$, exactly a $1-p^{-2}$ fraction of the affine
mixtures of any two of these inputs are nearby, and a $1-2p^{-2}$
fraction have singleton lists. The source separation remains of order
$\sqrt n$, at a radius covered by an explicit finite first-order
certificate. The far inputs form an affine $\F_p$-line; the random
mixing coefficients here range over $\F_{p^3}$.

For one-word lists over $\F_{p^3}$, a classical norm-one construction
and finite DKT certificates give matching linear growth in length:
with $N=2(p^2+p+1)$ and dimension three, the maximum list size is
$\Theta_c(N)$ at agreement $\lceil cp\rceil$ for every fixed
$\sqrt2<c<2$. This is a vanishing-rate extension-field comparison.
...
are not new. The distinction here is the simultaneous exact source and
common agreement and the coverage of all $p-2$ interior mixtures.
...
Allowing the message dimension to grow gives a polynomial source
separation. Theorem~\ref{thm:correlated-cover-line} constructs, for every
...
rates. Their distance from capacity is a different parameter from our
source separation $d/n=\Theta(\log n/(n\log\log n))$.
