...
an affine line can be nearby at every point but one, with the exception
only one coordinate short of the maximum possible distance from the code. Changing
...
At every fixed rational rate, we construct affine lines whose $p-1$
nonzero parameters are all nearby, while parameter zero is only one
coordinate short of the maximum possible Reed--Solomon distance.
The radius is strictly below Elias, and its gap is
$\eta\sim H_2(\rho)/\log_2p$. The far point lies $(1-o(1))\eta$
outside that radius, while the proposed numerical nearby fraction
tends to zero. The length can be $\Theta_\rho(\log p\log\log p)$. The same codes
also contain lines with two far endpoints and every other point nearby;
both endpoints can lie almost half a gap outside the tested radius.
Paired evaluation domains and a multiplicative-character estimate
produce this behavior; all nonzero points have exactly the tested
distance and have pairwise disjoint decoding lists. A separate cubic deformation gives complete
nonzero coverage at length $\Theta(\log p)$ against arbitrary fixed
constants in the proposed numerical bound. Both constructions output
the code and line in expected polynomial time in $\log p$, with
success probability $1-p^{-\Omega(1)}$.
A deterministic powers-of-two construction also violates the numerical
bound with unique, efficiently recoverable nearby witnesses and an
almost maximally far exceptional point. An extension using prime-order
orbits makes the separation arbitrarily close to the entire gap, on
suitable infinite progressions of prime fields, while retaining unique
and efficiently recoverable witnesses. Two padding orbits strengthen
this further: uniqueness and almost full-gap separation persist even
after doubling the proposed exponent constant. A separate existence
argument gives this strengthening at logarithmic length.
...
$r$ words can each lie almost an entire gap outside the radius while
a random affine combination is nearby with probability $1-O(r/p)$.
...
factor greater than $2^5$ over $\F_{2^{44497}-1}$.
