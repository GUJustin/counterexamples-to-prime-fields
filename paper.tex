...
prime fields. Even on short domains in large prime fields, the proposed list-size
exponent grows too slowly as the gap from capacity shrinks. Moreover,
an affine line can be nearby at every point but one, with that point
a fixed fraction of the capacity gap beyond the radius. Changing
universal constants cannot repair the proposed bounds.
...
At every fixed rational rate, we construct length-$\Theta(\log p)$
affine lines whose $p-1$ nonzero parameters are all nearby, while
parameter zero lies a fixed positive fraction of the capacity gap
outside the radius. The radius is strictly below Elias, the gap is
$\Theta(1/\log p)$, and there is no correlated agreement. In particular,
the far-point separation can be $\eta/2$ when the proposed numerical
nearby fraction tends to zero. A cubic change of domain preserves
moment equalities and makes the collision equations absolutely
irreducible; a point-count estimate then yields complete nonzero coverage.
The code and line can be sampled in expected polynomial time in
$\log p$, with success probability $1-p^{-\Omega(1)}$.
An anchored lift and padding construction
...
The clearest line-level message is that \emph{a line can be close to
the code everywhere except at one point, even when that point is half
the capacity gap beyond the radius}. At every fixed rational rate,
Theorem~\ref{cw:punctured-line} gives prime-field codes with
$n=\Theta(\log p)$ for which every nonzero parameter is nearby, while
parameter zero is $\eta/2$ outside. The radius is strictly below Elias,
and no pair of codewords explains the line on the required common
coordinate set. The $c_1=c_2=1$ numerical prescription predicts a nearby
fraction tending to zero; the actual fraction is $1-1/p$.
Thus testing a random linear combination can find a nearby word with
probability arbitrarily close to one despite this separated input and
the absence of correlated agreement. More generally, the theorem defeats
any fixed constants $c_1,c_2$, with a fixed positive separation fraction
depending on $c_2$.

The gap still shrinks as $\Theta(1/\log p)$. These results do not
...
Theorem~\ref{cw:punctured-line} gives complete nonzero coverage: all
$p-1$ nonzero parameters are nearby while the remaining point lies a
fixed positive fraction of $\eta$ outside. Matching three times as many
...
and a finite-field point-count estimate show that most outside values
are represented by the candidate polynomials. Padding then covers
...
use padded interval domains, whereas the complete-coverage theorem
uses cubic images of intervals with added coordinates.
...
can be. Theorem~\ref{cw:punctured-line} makes every nonzero parameter
nearby, with a point $\eta/2$ outside at $c_1=c_2=1$, while the proposed
numerical fraction tends to zero. Its length-$214$, half-rate certificate over $2^{127}-1$ compares
...
Can the full punctured-line behavior of Theorem~\ref{cw:punctured-line}
...
It also has very large global lists. The fixed-gap construction gives
whole-line uniqueness, but only a linear lower bound on the nearby count.
\item \textbf{Recovering nearby witnesses.}
Can one efficiently recover nearby codewords for the displayed lines,
or enumerate enough of the equal-moment classes to match the counting
lower bounds? Proposition~\ref{cw:sampling} outputs the code and line
in expected polynomial time in $\log p$, but does not recover a witness
