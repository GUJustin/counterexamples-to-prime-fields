...
prime fields. The main obstruction is their dependence on the gap from
capacity: even on short domains in large prime fields, the proposed
list-size exponent grows too slowly as the gap shrinks. Changing
universal constants cannot repair it.
...
affine line. At polylogarithmic block length and gap
$\Theta(1/\log p)$, the fraction of nearby field parameters can tend to
one, even though the line contains a word one coordinate beyond the
nearby radius, with no correlated agreement. An anchored lift and padding construction
gives the same failure at an exactly fixed positive rate and gap, at
arbitrarily large lengths, even when every nearby point on the line
has a unique nearby codeword. The necessary coefficient of a linear
exceptional-count bound has logarithm
$\Omega_\rho(\eta^{-2}/\log(1/\eta))$ at every sufficiently small
rational gap.
...
the quantitative coding assumptions used in the S-two whitepaper.
The stated uniform list prescription is false. The proposed numerical
line coefficient also fails on fixed-gap families, against every
remainder sublinear along those families. We distinguish this conclusion
from a conjecture using actual list size with a remainder controlled
only at each fixed gap. These results invalidate the corresponding
quantitative premises; questions about the soundness of a complete
...
characteristic-based Elias radius. The line constructions give a
corresponding obstruction to the proposed coefficient multiplying
$n$ in an exceptional-parameter bound. The failure can be nearly total:
at polylogarithmic length and gap $\Theta(1/\log p)$, the fraction of
nearby parameters tends to one, although the line contains a word one
coordinate beyond the radius and no common agreement explanation exists. Choosing the gap constant appropriately makes the proposed
fraction tend to zero. Thus being close to the code for a random linear
combination need not indicate common agreement on a large coordinate set.
These results do not establish superlinear growth in $n$ at one fixed
positive gap on short prime-field domains; that stronger question remains
open.
...
list give different parameters $z$. Theorem~\ref{thm:logarithmic-lines}
also gives such counterexamples over every sufficiently large prime $p$,
with block length $\Theta(\log p)$ and $\Omega(p/\log p)$ nearby
labels. Its gap is $\Theta(1/\log p)$.
A padding refinement (Theorem~\ref{fg:dense-lines}) is stronger:
for any fixed fraction below one, that fraction of all field parameters
can be nearby, with no correlated agreement, at length
$\Theta(\log p)$ and gap $\Theta(1/\sqrt{\log p\,\log\log p})$.
The proposed numerical bound predicts only $p^{o(1)}$ nearby parameters
there, compared with an actual count of order $p$.
Theorem~\ref{fg:near-unit-density} makes the nearby fraction tend to one,
at polylogarithmic length and gap $\Theta(1/\log p)$, while retaining a
word one coordinate beyond the radius. Choosing its constant appropriately
makes the proposed fraction tend to zero.
...
from the still-open fixed-gap quadratic proximity-gap lower bound.
...
Every nearby word on the line has exactly one nearby codeword.
...
$D_{\rm old}\cup\{1\}$ at which all values
$\{P(x_j):P\in\mathcal T\}$ are pairwise distinct.
There are enough such points if
\[
 p>mB+(k_B-1)\binom M2+q_B,
\]
by the root bound for pairwise differences in $\mathcal T$.
Choose offsets $b_j$ successively so that all labels
$P(x_j)-b_j$, for $P\in\mathcal T$ and all $j$, are distinct.
At step $j$, at most $(j-1)M^2$ offsets are forbidden, so
$p>(q_B-1)M^2$ suffices.
On these new coordinates set $f(x_j)=b_j$ and $g(x_j)=1$.
At $z_{ij}=P_i(x_j)-b_j$, the polynomial $P_i$ agrees at its $A_B-1$ old
...
every nearby word on the line has a unique nearby codeword,
...
\]
Indeed, the candidates have degree at most $B(k-1)-1$, and the two
common-agreement bounds are $Bt-1$ and $K-1+q$.
The inequality $Bt-q\ge K$ also preserves the full candidate-pool
...
This assertion does not include uniqueness of nearby codewords.
\end{corollary}
\begin{proof}
...
are then
\[
 p>mB+(K_B-1)\binom L2+q_B,\qquad p>(q_B-1)L^2.
\]
Both are eventually implied by the displayed lower bound on $p$,
since the seed and $L$ are fixed and $K_B,q_B=O(B)$. This gives
$Lq_B>C_\rho(\eta)n_B$ distinct nearby labels. The two degree bounds
excluding correlated agreement do not change. Restoring the anchor
