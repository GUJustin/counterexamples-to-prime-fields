...
At message dimension three, an asymmetric quadratic intersection bank
and a translated grid give a positive constant fraction of singleton
exceptional challenges over primes $p=\Theta(n(\log n)^4)$, with source
separation $\Theta(n^{1/3}/(\log n)^2)$ coordinates, in the regime of the
Dao--Kominers--Thaler bounds. The distance below capacity is
$\Theta(n^{-1/2})$ and the proximity loss is
$\Theta(n^{-2/3}/(\log n)^2)$. This is a vanishing-rate result, not
fixed-rate or quadratic-count tightness.
