...
Puncturing at most $3p-2$ coordinates then makes two inputs individually
far while preserving singleton lists for at least a $1-1/p$ fraction
of their affine mixtures over $\F_{p^3}$. The source separation remains
of order $\sqrt n$, at a radius covered by an explicit finite
...
Theorem~\ref{thm:fp3-far-endpoints} upgrades this construction to two
individually far inputs over the same field. Choose two parameters
with the same unique nearby quadratic and delete its agreement
supports. At most $3p-2$ coordinates suffice to erase both nearby
witnesses. For every parameter outside one $p^2$-point plane, the
nearby quadratic has a different leading coefficient and loses at
most six matches. Hence both inputs have agreement at most
$p+2\sqrt p$, while at least $p^3-p^2$ affine mixtures have singleton
lists at threshold $2p-\lceil4\sqrt p\rceil-8$.
The length remains $2p^2-O(p)$, and the finite first-order certificate
still applies for $p\ge4099$. Thus the exceptional probability tends
to one with a source separation of order $\sqrt n$ and characteristic
of order $\sqrt n$. This improves the far-input extension-field
parameters; it remains a vanishing-rate result over $\F_{p^3}$.
