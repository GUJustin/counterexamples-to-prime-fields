...
\date{Working draft --- September 18, 2026}
\begin{document}
\maketitle
\begin{abstract}
We give counterexamples to proposed list-decoding and proximity-gap
bounds for Reed--Solomon codes over prime fields. The central line-level
phenomenon is that, even strictly below the characteristic-based Elias
radius, two far words can have every other affine combination close to
the code. The nearby polynomial can change with the mixing coefficient;
a nearby mixture need not reflect common agreement of the inputs.

Write $\theta=1-\rho-\eta$ for the tested radius at rate $\rho$.
At every fixed limiting rate, we construct polynomial-size prime
alphabets $p=\Theta((n/\log n)^\beta)$, for any fixed $\beta>12/5$,
with $\eta=\Theta(1/\log n)$. Both endpoints have distance
$\theta+(1/2+o(1))\eta$, while all $p-2$ interior affine mixtures
have distance exactly $\theta$. The domains are selected unions of
multiplicative cosets. A second family works over every sufficiently
large odd prime, with $n=\Theta(\log p)$ and endpoint separation
exactly $2\eta/3$. An explicit length-48, dimension-24 example over
$\mathbb F_{65537}$ has $\eta=1/16$ and endpoint separation $1/24$;
complete coverage is independently verified with integer arithmetic.
These statements sharpen the line profile at the already known
$1/\log p$ capacity-margin scale, rather than establishing a new gap order.

A separate paired-domain construction has an almost maximally far
point, all other affine parameters nearby, and pairwise disjoint
nearby lists. It extends to batches consisting entirely of far inputs.
Other constructions violate numerical line bounds with singleton
lists and efficient witness recovery, so decoding ambiguity is not
required for these failures.

We also disprove uniform list-size prescriptions. At rate $1/4$ and
fixed gap $1/8$, full-length prime-field codes have lists of size $n/2$.
Integer-interval domains give
$\log L=\Omega_\rho(\eta^{-2}/\log(1/\eta))$ strictly below Elias,
ruling out every uniform $\exp(O_\rho(1/\eta))$ bound.
At message dimension three, two constructions give a positive constant
fraction of singleton exceptional challenges in the regime of the
Dao--Kominers--Thaler bounds. A block construction has $p=\Theta(n)$
and proximity loss $\Theta(n^{-1/2})$, a constant fraction of its
$\Theta(n^{-1/2})$ capacity margin, but only $\Theta(n)$ exceptional
challenges. An asymmetric intersection bank and translated grid give
$p=\Theta(n(\log n)^4)$ and loss
$\Theta(n^{-2/3}/(\log n)^2)$, with a superlinear exceptional count.
These are vanishing-rate results, not fixed-rate or quadratic-count
tightness.

We distinguish fixed-gap from shrinking-gap claims, prime from
extension-field challenges, and chosen from prescribed domains when
applying the results to the conjectures in the S-two whitepaper.
The results do not by themselves give a concrete-security bound for
an entire deployed protocol.
\end{abstract}
...
Relaxing the length gives a complete eight-member nearest list: at
$n=32$ and dimension $k=7$, all eight codewords have exactly $14$
agreements, and no other codeword attains that agreement
(Theorem~\ref{thm:eight-cubic-smooth-seed}). This holds over arbitrarily
large prime fields and lies strictly above the first-order curve at
rate $7/32$. The construction gives an explicit rational sixteen-node cubic
incidence configuration and proves completeness by exact interpolation
and a quadratic norm calculation. It is a finite seed; growing the
list while preserving this agreement regime remains open.
Shared intersections of two additional cubics strengthen this to a complete
ten-member nearest list on eighteen nodes, with seven agreements each.
A degree-five pullback gives at least ten candidates at
$(n,k,A)=(90,16,35)$, strictly above the first-order curve
(Theorem~\ref{thm:ten-cubic-padding}). Completeness is proved for the
cubic source, but is not asserted for this pullback.
A triangle of shared intersections similarly gives a complete eleven-member
cubic list on nineteen nodes, with seven agreements each; its degree-thirteen
pullback has at least eleven candidates at $(n,k,A)=(247,40,91)$ above the
same curve (Theorem~\ref{thm:eleven-cubic-padding}). These finite extensions
do not yet give an unbounded-list family.
A separate incidence exchange gives a complete nine-member quartic list
with eight agreements on eighteen nodes. Its degree-twelve pullback
has at least nine nearby codewords at $(n,k,A)=(216,49,96)$, again above
the first-order curve (Theorem~\ref{thm:nine-quartic-exchange}). This step
removes two selected incidences before lifting and restores both at a
fresh intersection; it does not assert an indefinitely repeatable extension.
...
The paired-domain theorem uses a superpolynomial alphabet in the block
length. Theorem~\ref{thm:prime-random-fiber-log-gap} gives the two-far
profile instead over a polynomial-size prime alphabet, with every
non-endpoint mixture nearby at a radius strictly below Elias. Its
domain is a selected union of multiplicative cosets, not necessarily a
subgroup. Krachun--Kazanin--Hab\"ock already obtain polynomial prime
alphabets and below-Elias inverse-logarithmic gaps; those features alone
are not new. An origin shift in their quotient construction also gives
two far endpoints with the same asymptotic half-gap separation and exact
source/common agreement. The strengthening here is coverage of
\emph{all} $p-2$ interior mixtures in the polynomial-alphabet,
below-Elias regime, rather than only the large subset certified there.
Thus the comparisons concern quantitative coverage, separation, and reciprocal-gap dependence;
...
There is now a prime-field lower bound at error rates covered by the
Dao--Kominers--Thaler upper bounds, albeit at vanishing code rate. Theorem~\ref{thm:prime-quadratic-intersection-line}
constructs a code of dimension three and length $n=2L(L-1)+1$,
with more than $n^{3/2}/4$ exceptional challenges. Both source agreements
and common agreement are $2L-2$, but every exceptional word has a
singleton list at threshold $T=2L-1$. This threshold is above the
first-order curve and below exact Johnson: $T^2=2n-1$.
The construction works over every sufficiently large prime field, with
a stated polynomial field-size bound, and an explicit finite certificate
gives the uniform upper comparison $31000n^2$ for full-agreement-set
MCA exceptions. Thus \emph{even over prime fields, many exceptional
challenges can coexist with singleton lists at error rates covered by
the first-derivative upper bounds}.
Corollary~\ref{cor:prime-positive-density} strengthens the field-size
accounting: for an infinite sequence of primes, $n=\Theta(p^{2/3})$
and more than $p/36$ challenges have singleton threshold lists. Thus the
exceptional probability can stay bounded away from zero despite these
lists being singletons.
The rate is $3/n$, and the source common agreement misses the tested
threshold by only one coordinate. This is a superlinear lower bound in
the applicable regime, not fixed-rate or fixed-gap tightness.

The one-coordinate restriction can be removed without increasing the
message dimension. Theorem~\ref{thm:growing-gap-random-line} gives, for
every sufficiently large prime, two words whose individual and common
agreements are exactly $A$, while a constant fraction of their linear
combinations have exactly one codeword with at least $A+d$ agreements.
Here $d$ grows on the order of $\log n/\log\log n$. For example, one
choice gives $d\sim\log n/(2\log\log n)$ and
$\Omega(n\log\log n)$ singleton exceptional challenges.
The construction replaces polynomial padding by independent random values
with the two source lists excluded, and uses a multiplicative Sidon set
to realize the core over small prime fields. Its scope remains vanishing
rate and vanishing fractional gap; it does not prove fixed-rate tightness.
Moreover, on these positive-density examples $p=n\,\mathrm{polylog}(n)$,
so the uniform $O(n^2)$ exceptional-count upper bound exceeds the entire
field and gives no nontrivial probability bound. The construction therefore
does not establish sharpness of that quadratic upper bound.

A translated integer grid makes the source separation polynomial
while retaining message dimension three
(Theorem~\ref{thm:translated-grid-polynomial-gap}). It gives
$d=\Theta(n^{1/6}/\log n)$ and
$\Omega(n^{4/3}\log n)$ singleton exceptional challenges over prime
fields of size $\Theta(n^{4/3}(\log n)^4)$. Both endpoints and ordinary
common agreement are exactly $d$ coordinates below the threshold.
The threshold remains above the first-order curve and strictly below
the degree-two Johnson agreement. The construction correlates the
fresh labels through short integer linear forms; it does not rely on
rare repeated hits from independent random values. More generally its
count attains $\Omega(n^{3/2}/d)$ over a range of growing gaps.
The exceptional fraction is of order $(\log p)^{-3}$, rather than a
positive constant. These bounds strengthen the count--separation
tradeoff at vanishing rate; they do not establish quadratic-count
tightness or a practical fixed-domain improvement.

Allowing exactly controlled collisions in the bank improves the gap
and the exceptional fraction, with a different absolute-count tradeoff
(Theorem~\ref{thm:collision-bank-constant-fraction}). Two bank pairs may
share a squared intersection coordinate; its two roots are assigned
separately to the pairs. Two disjoint complete bipartite graphs make the
resulting core agreement exactly equal for every bank polynomial.
The construction gives $d=\Theta(n^{1/4}/\log n)$ and at least
$2^{-19}p$ singleton exceptional challenges, where
$p=\Theta(n^{5/4}\log n)$, still at dimension three with exact endpoint
and ordinary common agreement $A$. Its capacity margin is
$(T-3)/n=\Theta(n^{-1/2})$, while its proximity loss is
$d/n=\Theta(n^{-3/4}/\log n)$. Thus even this larger loss remains a
vanishing fraction of the capacity margin. The absolute exceptional-count
exponent is smaller than in the preceding construction; this is a new
tradeoff between these proved constructions, not superiority in every
parameter or an improvement over the known fixed-rate results.

Using two unequal components separates the task of creating the core
from that of supplying rich grid witnesses
(Theorem~\ref{thm:asymmetric-collision-bank}). The smaller component
supplies all nearby witnesses; the larger component supplies core
intersections but stays below the threshold at every challenge.
This gives a constant fraction of singleton exceptional labels with
$p=\Theta(n(\log n)^4)$ and
$d=\Theta(n^{1/3}/(\log n)^2)$. The list at the relaxed source threshold
has size at most $R+1$, where $R=\Theta(n^{1/3}(\log n)^2)$.
The capacity margin remains $\Theta(n^{-1/2})$, and the proximity loss
is $\Theta(n^{-2/3}/(\log n)^2)$; their ratio still tends to zero.
This improves the separation and decreases the required alphabet size,
while giving a smaller absolute exceptional count than the preceding
polynomial-count constructions. Within one translated grid with
$M=O(\sqrt n)$ and a constant fraction of prime-field labels, the gap
must satisfy $d=O(n^{1/3})$
(Proposition~\ref{prop:translated-grid-gap-ceiling}); the asymmetric
construction attains this exponent up to logarithms. This ceiling applies
to that grid mechanism, not to arbitrary Reed--Solomon counterexamples.

A different block construction permits a loss comparable to the
capacity margin, at the cost of a linear exceptional count
(Theorem~\ref{thm:square-linear-block-bank}). Multiples of $X^2$ and
constant polynomials form the bank; on each fresh block the direction is
a scalar multiple of $X^2$. Scalar copies let distinct challenge labels
use the same geometric line, so the preceding fixed-bank incidence
obstruction does not apply. Explicitly, $n=128t^2+1$, both endpoint
agreements and ordinary common agreement are $A=14t$, and exactly
$t^2+1$ affine challenges have singleton lists at threshold $T=16t$.
Both $A$ and $T$ are above first order, and $T$ is below Johnson.
The loss-to-capacity-margin ratio tends to $1/8$. Choosing $p=\Theta(n)$
makes the exceptional fraction positive; over larger odd fields the
same construction gives probability of order $n/q$. Its linear count
does not establish tightness of an $O(n^2/q)$ bound, and this is not
a separation between prime and extension fields.

A complementary construction allows the message dimension to grow
and gives a polynomial source separation. Theorem~\ref{thm:correlated-cover-line} constructs, for every
fixed $0<\alpha\le1/3$, examples with dimension and source separation
$\Theta(n^\alpha)$ and $\Theta(n^{3(1-\alpha)/2})$ singleton exceptional
challenges. For example, separation $\Theta(n^{1/5})$ coexists with
$\Theta(n^{6/5})$ exceptions. The construction groups coordinates into
complete fibers of $x\mapsto x^s$ and makes their fresh values agree.
A separate argument excludes all new polynomial witnesses introduced by
the larger code dimension. The fractional source gap and code rate still
tend to zero; this does not prove fixed-rate tightness.
Over sufficiently large primes, a deterministic variant makes every list
above the source agreement empty or a singleton, while attaining the
separate conic-witness upper bound up to a constant factor
(Proposition~\ref{prop:conic-bank-order-sharpness}). This is a statement
about all points on the constructed line, not about all received words:
other received words for the same code can have at least $L$ witnesses
at the tested threshold.

These statements should not be read as the first superlinear prime-field
proximity-gap counterexamples. Diamond and Gruen~\cite{diamond-gruen-random}
refute the capacity conjecture using random-word distance distributions.
Krachun, Kazanin, and Hab\"ock~\cite{kkh}
give prime-field counterexamples at distance
$1-\rho-\Theta_\rho(1/\log n)$ from the code, including fixed positive
rates. For the translated-grid construction, the rate is $3/n$,
the distance below the capacity radius is
$(T-3)/n=\Theta(n^{-1/2})$, and the source-to-threshold
proximity loss is $(T-A)/n=\Theta(n^{-5/6}/\log n)$.
These are different parameters: neither the larger absolute source
separation compared with our earlier construction nor the superlinear
challenge count establishes an improvement over the known
inverse-logarithmic capacity-gap results. In particular, we do not claim
fixed-rate tightness or an improvement to a practical security estimate.
Our examples lie in the range of decoding radii covered by the
first-derivative upper bounds of Dao, Kominers, and Thaler~\cite{dkt2026}.
Their purpose is to lower-bound the number of exceptional challenges
within that range, even when the relevant lists are singletons.
This does not supersede those fixed-rate near-capacity
results. Throughout this comparison, ``below Johnson'' for an agreement
threshold means a decoding radius beyond the Johnson radius.

An independent projective construction in
Theorem~\ref{thm:projective-quadratic-line} gives the same $3/2$
exponent over extension fields with an exact two-level agreement spectrum:
all exceptional words have agreement $2(p^2+p+1)$ and every other word
has agreement $2(p+1)$. Its affine-witness companion in
Corollary~\ref{cor:projective-linear-sharp-packing} asymptotically
attains the universal triple-count bound with constant one. That separate
sharpness statement is not above the rate-only first-order curve.

Proposition~\ref{prop:scaled-frobenius-padding} improves the characteristic
scale of the extension-field construction from $\Theta(n^{1/4})$ to
$\Theta(\sqrt n)$, retaining the $n^{3/2}$ exceptional-count scale and
$\sqrt n$ agreement-gap scale. Explicitly, over $\F_{p^4}$ it has
$n=9p^2$, dimension three, and threshold $4p$, while both source
agreements and common agreement are exactly $2p$. There are exactly
$(p+1)(p-1)^2$ singleton exceptions and one two-word exception.
The construction satisfies an explicit finite DKT certificate whose
characteristic condition is only $p>3$. Its ambient field has size
$n^2/81$. This improves the characteristic and field-size parameters;
it does not increase the known exceptional-count exponent or yield
prime-alphabet, fixed-rate, or general-bound tightness.

Puncturing strengthens the singleton conclusion
(Proposition~\ref{prop:quarter-frobenius-block}): at length
$n=(5p^2-1)/2$, the same three-symbol message space has
$(p+1)(p^2-1)$ singleton exceptions at the largest integer agreement
strictly below Johnson, while both source agreements remain $2p$ and
lie above the first-order curve. The loss is asymptotically
$1-2/\sqrt5$ of the margin above capacity.
Here dimension three means encoding only three field elements as a
codeword of length $\Theta(p^2)$: the rate is $3/n$, and the fractional
agreement loss is $\Theta(1/p)$. Thus this is a vanishing-rate theorem,
not a constant-rate counterexample on a practical evaluation domain.
Corollary~\ref{cor:quarter-frobenius-growing-dimension} raises the message
dimension to $\lfloor p/20\rfloor+1$ while preserving the complete
exceptional-list profile and a positive loss-to-capacity-margin ratio.
Its rate still vanishes, and its threshold falls below the enlarged
code's first-order curve; this tradeoff does not remove the main
limitation of the dimension-three theorem.

A higher-power construction gives two far inputs with maximum agreement
exactly $hp$, yet $n^{3/2-o(1)}$ of their affine mixtures each have
at least $(\sqrt2+o(1))hp$ matches and a unique qualifying codeword
(Theorem~\ref{thm:higher-power-frobenius-window}). The message dimension
is $h+1\to\infty$. The source agreements approach the first-order
boundary and the tested agreement approaches Johnson, so the loss spans
asymptotically the whole interval between these agreement thresholds.
As a fraction of the margin above capacity, the loss tends to
$1-1/\sqrt2\approx29.3\%$.
This is an extension-field result over $\F_{p^4}$ with rate
$n^{-1+o(1)}$ and an explicitly specified domain. Even its degree-four BabyBear
specialization, with guaranteed relative loss above $28.965\%$, uses
about $9.8\times10^{22}$ coordinates. It gives neither a better.codes
improvement nor a security ceiling for a practical proof system.

A high-rate norm family provides a complementary tradeoff
(Theorem~\ref{thm:odd-hermitian-high-rate}). Over $\mathbb F_{p^4}$,
length $n=p^2$ and dimension $n-2p$ admit exactly $n^{3/2}-n$ exceptional
affine parameters, all with singleton lists, and both inputs individually far. The common
agreement loss is $p-2$, while the smaller guaranteed individual
loss has order $\sqrt p$. The threshold is below Johnson and below
the first-order curve, and the dimension exceeds the characteristic.
Over $\mathbb F_{p^8}$ both individual losses equal the whole capacity
margin. Padding the domain also gives any fixed rate in $(0,1)$,
with $\Theta(n^{3/2})$ exceptional parameters and both source agreements
exactly at capacity (Corollary~\ref{cor:hermitian-fixed-rate-padding}).
The fractional margin is only $\Theta(n^{-1/2})$, the alphabet is
$\mathbb F_{p^8}$, and the domain still has order $p^2$; the padded
statement does not assert singleton lists.
The two specific quartic-field inputs cannot uniformly match this
individual separation: along an unconditional sequence of primes,
both individual losses are a vanishing fraction of the capacity margin
(Proposition~\ref{prop:hermitian-explicit-endpoint-limitation}).
This limitation concerns those chosen inputs, not every possible pair
on the line.

The existing odd-characteristic elliptic construction also admits an
exact finite refinement (Theorem~\ref{thm:odd-trace-norm-refinement}).
Over $\mathbb F_{3^8}$, a length-$81$, dimension-$34$ example has
exactly $648$ exceptional challenges, each with a singleton list and
agreement exactly $51$, while the two source agreements are at most
$45$ and exactly $34$. Every other challenge has agreement at most $50$.
Its threshold is above first order and below Johnson. The refinement
uses the exact correction degree, removes label collisions, and classifies
all threshold witnesses, including witnesses over the extension alphabet;
the fixed-characteristic asymptotic regime is already present
in~\cite{binary-counterexamples}.
The analogous odd-dimensional Gold family has exactly $n(n-1)/2$
singleton exceptions at every exterior pole in a cubic extension of
its evaluation field (Theorem~\ref{thm:odd-gold-exact-classification}).
For example, over $\mathbb F_{3^{15}}$, length $243$ and dimension $102$
give exactly $29{,}403$ exceptions of agreement $153$, with source
agreements at most $135$ and exactly $102$; all other parameters have
agreement at most $152$. The quadratic population is inherited;
the exact count at every cubic pole and exhaustive lists are the
refinements. Over a quadratic extension of the same evaluation field,
an exact one-dimensional evaluation kernel instead gives
$\Theta(n^2/p)$ exceptional parameters, each with at most $p$ threshold
witnesses, with the same individually far sources
(Corollary~\ref{cor:odd-gold-quadratic-kernel}).
This bound is uniform as the characteristic grows; at $n=p^5$ it gives
$\Theta(p^9)$ exceptions over $\mathbb F_{p^{10}}$.
At $p=3$ in this quadratic alphabet, the exact count is $21{,}356$
exceptions out of $59{,}049$ affine parameters, with lists of size
at most three. These families use full extension-field domains.

For ordinary common agreement, Theorem~\ref{thm:fp3-common-agreement}
gives a stronger probability statement over $\F_{p^3}$. With
$n=2p^2-p-1$ and dimension three, every affine mixture has agreement
at least $2p-\lceil4\sqrt p\rceil-2$, while the two inputs have
ordinary common agreement at most $p+2\sqrt p$.
Exactly $p^3-p$ of the $p^3$ affine parameters have singleton lists;
the remaining $p$ have lists of size $p$ at the stated threshold.
The threshold satisfies an explicit finite first-order certificate
and is below the exact Johnson agreement threshold for primes $p\ge4099$.
Thus the common-agreement deficit has order $\sqrt n$ even when
every mixture is nearby and all but a $1/p^2$ fraction have unique
witnesses. The inputs themselves are nearby, so this result addresses
common agreement rather than separation of individually far sources.
The ambient field is an extension field of size $\Theta(n^{3/2})$;
the construction does not instantiate the fixed better.codes benchmark.

Theorem~\ref{thm:fp3-far-endpoints} upgrades this construction to
$p$ individually far inputs over the same field. Their unique nearby
quadratic is the same polynomial. Deleting just its core agreement
fiber, of size at most $p$, lowers their individual agreements to at
most $p+2\sqrt p$. Every other nearby quadratic loses at most two
matches, or has a disjoint core fiber. At threshold
$2p-\lceil4\sqrt p\rceil-4$, the exact affine-line spectrum becomes
$p$ empty lists, $p^3-2p$ singleton lists, and $p$ lists of size $p$.
The length remains $2p^2-O(p)$, and the finite first-order certificate
still applies for $p\ge4099$. Thus \emph{two far inputs have a nearby
mixture with probability exactly $1-p^{-2}$}, with source separation
and characteristic both of order $\sqrt n$.
The far parameters form an affine $\F_p$-line, so combinations with
$\F_p$ coefficients stay far; the high failure probability uses
coefficients from $\F_{p^3}$. This is a vanishing-rate extension-field
result, not a prime-alphabet assertion.

For list size itself, Theorem~\ref{thm:norm-one-direct-list} gives a
matched constant-factor comparison. At length $N=2(p^2+p+1)$ over
$\F_{p^3}$, dimension three, and threshold
$T=\lceil19p/10\rceil$ for $p\ge53$, an explicit word has exactly $N/2$
nearby quadratics, while an elementary shortening bound limits every
word's list to $4N/3$. The explicit list is therefore within a factor
$8/3$ of the universal maximum at these parameters. The threshold stays
a constant times $\sqrt N$ away from both the first-order curve and
the Johnson boundary. Corollary~\ref{cor:norm-one-full-interior-band} extends the matched
linear growth to every fixed threshold $T=\lceil cp\rceil$ with
$\sqrt3<c<2$: for $p\ge53$ and every integer threshold from
$\lceil\sqrt3\,p\rceil$ through $2p+2$, the same word has $N/2$
witnesses and the uniform upper bound is $9N/4$. Thus linear list growth is
necessary throughout the asymptotic interior between the two boundaries,
rather than only at one chosen threshold. More generally,
Corollary~\ref{cor:norm-one-general-range} gives $\Theta_c(N)$ lists
for every fixed $\sqrt2<c<2$, with universal upper bound
$\bigl(2/(c^2-2)+o_c(1)\bigr)N$. The resulting upper-to-lower ratio
is between two and four in the strict asymptotic first-order/Johnson
interior. The additional range below $\sqrt3$
is a finite-degree consequence, rather than a change to the rate-only
first-order curve. The example uses
classical projective-plane incidence geometry; its rate and normalized
agreement vanish. This establishes neither prime-alphabet tightness nor
tightness of the received-line exceptional count.

For a structured algebraic realization at fixed rate,
Theorem~\ref{thm:growing-characteristic-native} uses length $N=p^5$
over $\mathbb F_{p^5}$ and gives at least $N-3$ nearby native challenges,
with common-agreement gap $\Theta_\rho(N^{3/5})$; on an infinite
sequence of primes every challenge is nearby. It combines the locator
compiler of~\cite{binary-counterexamples} with internal padding,
a published scattered-set exclusion, and an exact cubic criterion for
the remaining labels. Here characteristic is $N^{1/5}$, but is smaller
than the message degree. The first source is nearby and the direction
is far. As an existence statement, its count and asymptotic gap are
subsumed by Crites--Stewart~\cite[Corollary~1]{crites-stewart2025},
even with the same full domain and exact common agreement.
It is not a new existential regime or a large-characteristic,
above-first-order comparison.

A norm-quotient specialization gives a stronger gap with two far sources
(Theorem~\ref{thm:native-norm-quotient}). For every fixed extension degree
$d\ge2$, length $q=p^d$, and exact dimension $J=\lfloor\rho q\rfloor$,
put $m=(q-1)/(p-1)$. Each source has agreement $J+m-1$, and their common
agreement is also $J+m-1$, while all $q-2$ nonendpoint affine combinations have at least
$J+2m-1$ agreements. Thus the common-agreement loss is about $1/p$,
half the approximately $2/p$ distance below capacity. This combines the
standard quotient construction with Katz's character-sum estimate;
it still has a smaller capacity margin than the inverse-logarithmic
counterexamples in~\cite{kkh}. Its full-field domain and extension
alphabet do not establish the practical benchmark or prime-alphabet
tightness.
Moreover, Crites--Stewart already give every affine challenge nearby
at inverse-logarithmic margin over arbitrary fields and domains.
The additional property of the norm construction is its two far
sources with exact source and common agreement; novelty of that
refinement is not established here.
By choosing only logarithmically many norm fibers,
Theorem~\ref{thm:random-norm-tag-log-gap} obtains the same two-far-source
profile with a common-agreement loss of order $1/\log n$, asymptotically
half its distance below capacity. Here
$n=\Theta(p^{d-1}\log p)$; for quadratic extensions the characteristic
is $\Theta(n/\log n)$. This matches the known order of capacity margin,
while retaining the extension-alphabet and specially chosen-domain
limitations just described.
Theorem~\ref{thm:prime-random-fiber-log-gap} also removes the
extension-alphabet requirement: for every fixed $\beta>12/5$, it gives
prime alphabets $p=\Theta((n/\log n)^\beta)$ on selected unions of
multiplicative cosets, with the same two-far profile and inverse-logarithmic
gap. More precisely, for every $C>1/h(\rho)$, where
$h(\rho)=-\rho\log\rho-(1-\rho)\log(1-\rho)$, its distance below
capacity is $(2/C+o(1))/\log p$ and its source/common-agreement loss is
$(1/C+o(1))/\log p$. Choosing $C<2/h(\rho)$ places the tested radius
strictly below Elias. The proof first covers almost all multiplicative
labels with fixed-size products, then fills the remaining holes by
adding a constant number of pairs of alternatives. Each representation
uses exactly one factor from each pair, preserving its required degree.
This retains the earlier paired theorem's below-Elias two-far behavior
while allowing a polynomial prime alphabet. It does not establish
fixed-rate first-order tightness or settle a prescribed-domain benchmark.
An exact below-Elias certificate over $\mathbb F_{65537}$ has length
$48$, dimension $24$, source/common agreement $25$, and agreement
exactly $27$ at every nonzero pencil label. Its distance below capacity
is $1/16$, and its source-to-near distance loss is $1/24$.
Corollary~\ref{cor:two-point-fiber-all-primes} gives the same ratio
$2/3$ between loss and capacity margin over every sufficiently large
odd prime, with $n=\Theta(\log p)$ and radius strictly below Elias.
This last family has exponential alphabet size in $n$; it is distinct
from the polynomial-alphabet family above.
Novelty of the two-far refinement relative to the wider literature
...
\input{research/growing_m/proposed_gap.tex}
...
\input{research/dickson_fixed_gap/comparison.tex}
...
\input{research/logarithmic_length_lines/proposed.tex}
...
\input{research/fixed_gap_padding/averaged.tex}
...
\input{research/overnight_2026-09-16/finite_updates.tex}
...
\input{research/binary_affine_locator/multiplicative_orbit_appendix.tex}
\input{research/binary_affine_locator/multiplicative_label_appendix.tex}
\input{research/bounded_root_mca/appendix.tex}
\input{research/first_order_actual_components/appendix.tex}
\input{research/quasilinear_first_order/appendix.tex}
\input{research/isolated_solution_sharpness/appendix.tex}
\input{research/actual_higher_order_components/appendix.tex}
\input{research/inverse_bernoulli/appendix.tex}
\input{research/structured_domains/general_cubic_packets/appendix.tex}

\input{research/riccati_cross_ratio/appendix.tex}

\input{research/ordinary_ca_superlinear/appendix.tex}
\input{research/binary_characteristic_transfer/quadratic_frobenius_core/higher_power_frobenius.tex}
\input{research/binary_characteristic_transfer/odd_hermitian_high_rate.tex}
\input{research/binary_characteristic_transfer/hermitian_followups.tex}
\input{research/binary_characteristic_transfer/odd_trace_norm_refinement.tex}
\input{research/binary_characteristic_transfer/odd_gold_exact_classification.tex}
\input{research/binary_characteristic_transfer/quadratic_frobenius_core/norm_one_direct_list.tex}
\input{research/binary_characteristic_transfer/growing_characteristic_native.tex}
\input{research/norm_quotient_coverage/norm_quotient.tex}
\input{research/norm_quotient_coverage/random_tag_log_gap.tex}
\input{research/norm_quotient_coverage/prime_random_fiber_log_gap.tex}
\input{research/norm_quotient_coverage/extension_native_short_domains.tex}
\input{research/norm_quotient_coverage/target_subset_norm_compiler.tex}
\input{research/norm_quotient_coverage/fixed_ntt_discriminator.tex}

\input{research/quadratic_indicator_nearest/appendix.tex}

\input{research/two_branch_recurrence/appendix.tex}

\input{research/fixed_singular_cover/appendix.tex}

\input{research/hermite_johnson_route/appendix.tex}

\input{research/better_codes_second_jet/contact_resource_appendix.tex}

\input{research/prime_line_strengthening/four_word_one_pole/transversal_search/six_word_lift.tex}
\input{research/prime_line_strengthening/fano_seven/compactpaley_seven_appendix.tex}
\input{research/prime_line_strengthening/fano_seven/global_seven_appendix.tex}
\input{research/prime_line_strengthening/fano_seven/seven_bank_augmentation_obstruction.tex}
\input{research/prime_line_strengthening/octic_three_pole_route/quarter_rate_small_cover_addendum.tex}
\input{research/prime_line_strengthening/degree_ten_four_pole_route/paley_net_rational_exclusion.tex}
\input{research/relaxed_eight_cubic_route/eight_cubic_smooth_seed.tex}
\input{research/relaxed_eight_cubic_route/ten_cubic_padding.tex}
\input{research/relaxed_eight_cubic_route/eleven_cubic_padding.tex}
\input{research/relaxed_eight_cubic_route/padded_quadratic_completeness.tex}
\input{research/relaxed_eight_cubic_route/cubic_pullback_completeness.tex}
\input{research/relaxed_eight_cubic_route/nine_quartic_exchange.tex}
...
\bibitem{katz1989-character-sums} N. M. Katz.
\emph{An estimate for character sums}.
Journal of the American Mathematical Society 2(2), 197--200, 1989.
\url{https://web.math.princeton.edu/~nmk/old/estcharsums.pdf}.
\bibitem{crites-stewart2025} E. Crites and A. Stewart.
\emph{On Reed--Solomon Proximity Gaps Conjectures}.
IACR ePrint 2025/2046. \url{https://eprint.iacr.org/2025/2046}.
\bibitem{thorner-zaman2021} J. Thorner and A. Zaman.
\emph{Refinements to the prime number theorem for arithmetic progressions}.
arXiv:2108.10878, 2021.
\url{https://arxiv.org/abs/2108.10878}.
...
\bibitem{binary-counterexamples} Q. Dao, S. D. Kominers, J. Thaler,
and K. Z. Zheng.
\emph{Counterexamples to Beyond-Johnson Proximity Gaps over Binary Fields}.
Working manuscript, September 2026.
\url{https://github.com/GUJustin/binary_field_counterexamples}.
...
\bibitem{harper-bv} A. J. Harper.
Lecture notes 2 for Cambridge Part III course on Elementary Methods in
Analytic Number Theory, 2015. Theorem~7.1.
\url{https://warwick.ac.uk/fac/sci/maths/people/staff/harper/elementarymethodsnotes2.pdf}.

\bibitem{diamond-gruen-random} B. E. Diamond and A. Gruen.
\emph{On the Distribution of the Distances of Random Words}.
Cryptology ePrint Archive, Report 2025/2010; full version of CRYPTO 2026.
\url{https://eprint.iacr.org/2025/2010}.

\bibitem{mz2022-linearsets} M. Montanucci and C. Zanella.
\emph{A class of linear sets in $\mathrm{PG}(1,q^5)$}.
Finite Fields and Their Applications 78 (2022), 101983.
\url{https://doi.org/10.1016/j.ffa.2021.101983}.

\bibitem{lu2026-scattered} S. Lu.
\emph{Completing the classification of maximum scattered linear sets
in $\mathrm{PG}(1,q^5)$}. Preprint, September 1, 2026.
Only the explicitly checked incidence and tangent arguments are used here.
\url{https://shunqilu.github.io/publications/classification_maximum_scattered_linear_sets_PG1q5.pdf}.
