...
\usepackage{amsmath,amssymb,amsthm,booktabs,array,enumitem}
...
\date{Working draft --- September 16, 2026}
\begin{document}
\maketitle
\begin{abstract}
Security analyses of hash-based SNARKs use quantitative statements about
list decoding and proximity gaps of error-correcting codes. We give
counterexamples to several proposed bounds for Reed--Solomon codes over
prime fields. The results concern the dependence on the code rate, the
gap from capacity, the field size, and the evaluation domain.

For every fixed rational rate $\rho\in(0,1)$, we construct codes with
$2^{\Omega_\rho(n)}$ codewords agreeing with one received word on
$(\rho+\eta)n$ coordinates, where
$\eta=\Theta_\rho(1/\sqrt{n\log n})$ and
$\log_2p=\Theta_\rho(\sqrt{n\log n})$. The domain is an integer
interval embedded in $\F_p$, and the radius lies strictly below the
characteristic-based Elias bound. Thus no
$\exp(O_\rho(1/\eta))$ list bound holds uniformly over prime fields
and evaluation domains. More strongly, over every sufficiently large
prime field, writing $b=\log_2p$, the construction gives a below-Elias
list with $\log_2L\ge(1/2-o(1))b^2/\log_2b$.
The construction follows by counting subsets
with matching moments. Concentration and exact polynomial-weight
certificates sharpen the finite bounds.

We also separate list size from the number of nearby points on an
affine line. A large list gives an affine line with many nearby points
and no correlated agreement. An anchored lift and padding construction
gives the same failure at an exactly fixed positive rate and gap, at
arbitrarily large lengths. The necessary coefficient of a linear
exceptional-count bound has logarithm
$\Omega_\rho(\eta^{-2}/\log(1/\eta))$ along a sequence of gaps.
Finally, at gap $1/n$, we construct codes whose maximum list size is
exactly $n-k-1$, but with $\binom n{k+1}$ uniquely nearby points on
one affine line. This parameter count attains an unconditional
puncturing bound at that radius. The same generic domains have an exact list-size
profile at every integer Hamming radius below $n-k$; at fixed positive
rates, this profile follows from known upper and lower bounds.

Finite constructions on the norm-one circle give examples in the
polynomial spaces used by Circle STARKs. We apply the general results to
the two quantitative coding conjectures used in the S-two whitepaper:
the stated uniform list prescription and the stated line-decodability
threshold are both contradicted under their published quantifiers. These
coding conclusions directly invalidate the corresponding premises of the
S-two security analysis; questions about the soundness of a complete
...
A large list can be converted into a line by dividing by $X-\tau$,
where $\tau$ lies outside the evaluation domain. This division lowers
the witness degree by one. It also makes the direction word
$1/(X-\tau)$ unable to agree with a low-degree polynomial on too many
points. A collision count ensures that many members of the original
list give different parameters $z$.
...
hardness results~\cite{ggg}. The results here use quantitative
counts on integer intervals to obtain the stated list and line
separations. The number-theoretic splitting theorem used in the lift is
also classical.
...
exponential in the reciprocal gap. The multiplicative structure of
their examples remains a separate advantage. Our finite circle
examples concern the evaluation spaces of Circle STARKs~\cite{circle}.
...
the remaining coding-theoretic questions. The appendices contain the
circle proofs, quotient calculations, and finite certificates.
...
A larger example over the same prime forces $c_2>22$.
...
For the large-index congruence classes below, full balance does force
the Galois hypothesis. This includes the field and subgroup in the
pinned better.codes instance.

\begin{theorem}[Balanced rational fibers at large Frobenius index]
...
\]
Let $R\in F(X)$ have degree $B\ge2$, no poles on $\mu_n$, and
exactly $B$ distinct points in each fiber of its restriction to $\mu_n$.
If $n>6(B-1)$, then $\overline F(X)/\overline F(R)$ is Galois.
Consequently $R$ has one of the two forms in
...
$\epsilon=1$. The theorem covers every integer $B\le43691$.
...
Add $q_B=Br+1$ distinct points $x_j$ outside $D_{\rm old}\cup\{1\}$
at which the $L$ values $P_i(x_j)$ are pairwise distinct.
There are enough such points if
\[
 p>mB+(k_B-1)\binom L2+q_B,
\]
by the root bound for the nonzero differences $P_i-P_{i'}$.
Choose offsets $b_j$ successively so that all labels
$z_{ij}=P_i(x_j)-b_j$ are distinct. At step $j$, at most
$(j-1)L^2$ offsets are forbidden, so $p>(q_B-1)L^2$ suffices.
On these new coordinates set $f(x_j)=b_j$ and $g(x_j)=1$.
At $z_{ij}$, the polynomial $P_i$ agrees at its $A_B-1$ old
coordinates and exactly one new coordinate. This supplies $Lq_B$
distinct nearby challenges on a domain of size $n_B$.

Suppose now that $F,G$ have degree less than $k_B$. If $G=0$,
their common agreement is confined to $D_{\rm old}$ and has size
at most $A_B-1$, by the degree of $F_{\rm old}-F$.
If $G\ne0$, it has at most $k_B-1$ zeros on $D_{\rm old}$, so
the common agreement has size at most
\[
 k_B-1+q_B=B(k+r)<Bt=A_B.
\]
Thus no common witnesses exist at the threshold.
...
and the nonzero-$G$ bound becomes $B(k-1+r)\le B(t-1)<A_B$
when $r\le t-k$. The larger capacity gap also preserves the
Elias inequality.
\end{proof}
...
There is a sequence of positive gaps $\eta\to0$ with the following
property. For each gap separately, there are prime-field
...
and the radius is strictly below the characteristic-based Elias radius.
The $o_\rho(1)$ is as the sequence of gaps tends to zero;
$C_\rho(\eta)$ is fixed as $B$ grows.
\end{theorem}
\begin{proof}
Let $b$ be the denominator of $\rho$, and let $m$ grow through
multiples of $b$. Set $c^2=2H$ and choose
\[
 s=\left\lfloor c\sqrt{m/\log_2m}\right\rfloor,\qquad
 r=b\left\lfloor\frac{s}{2b}\right\rfloor,\qquad
 k=\rho(m+r),\qquad t=k+s.
\]
For sufficiently large $m$, all these are integers with
$1\le r<s$, $1\le k<t<m$. Among the $t$-subsets containing $1$,
...
Since $k/m=\rho+O(s/m)$ and
$\binom{m-1}{t-1}=(t/m)\binom mt$, the estimate in the proof
of Theorem~\ref{im:all-list} gives
\[
 \log_2 L\ge (H-c^2/4-o(1))m=(H/2-o(1))m.
\]
Apply Lemma~\ref{fg:anchored-padding}. Its rate is exactly $\rho$,
its gap is $\eta=s/(m+r)$, and one can take
$C_\rho(\eta)=rL/(m+r)$. Here $r/(m+r)\sim\eta/2$ and
\[
 m=\frac{H+o(1)}{\eta^2\log_2(1/\eta)}.
\]
The logarithm of the prefactor $r/(m+r)$ is only
$O(\log(1/\eta))$. Substitution proves
\eqref{fg:anchored-coefficient}. Once the seed is fixed, $B$
can grow without changing the rate, gap, or coefficient.
\end{proof}
...
Each has a unique nearby codeword $p_z$.
...
nearby parameters, each with one nearby codeword.
...
the full lists at eight constructed centers. The $79{,}079$ allocation
...
along a sequence of gaps. Their selected witnesses also have
...
What list bounds hold when $p$ is polynomial in $n$, or when the domain
is restricted to a specified multiplicative subgroup or circle coset?
The interval construction separates a universal claim from these more
...
$\eta^{-2}/\log(1/\eta)$ along a sequence of gaps. Whether a fixed
positive gap permits superlinear growth in length remains open here.
Theorem~\ref{thm:power-family-linear-mca} excludes fixed powers of
bounded-degree bases as a route to such growth in sufficiently large
characteristic, even though a third-order equation for this family has
an actual solution variety of quadratic degree.
...
\item \textbf{Finding the large classes.}
Can one efficiently describe or enumerate enough of the equal-moment
or equal-coefficient classes to match the counting lower bounds?
The existence arguments alone do not provide such algorithms.
...
$0\le a_0<T\le n$ be integers. Suppose a word $f_1$ agrees with every
codeword of $C$ in at most $a_0$ coordinates. For any word $f_0$ and any
selection of codewords $p_z$, indexed by distinct scalars, with
$\operatorname{agr}_D(f_0+zf_1,p_z)\ge T$, every parametrized affine
codeword graph contains at most
\[
 \left\lfloor\frac{n-a_0}{T-a_0}\right\rfloor
\]
of the selected pairs $(z,p_z)$.
\end{lemma}
\begin{proof}
Fix a graph $P_0+zP_1$ containing $q$ selected pairs, and let $a$ count
the coordinates at which $f_0=P_0$ and $f_1=P_1$ simultaneously. Then
$a\le a_0$. At every other coordinate the affine equality holds for at
most one of the distinct selected scalars. Counting agreements gives
$qT\le qa+n-a$, so
\[
 q\le\frac{n-a}{T-a}
   =1+\frac{n-T}{T-a}
   \le\frac{n-a_0}{T-a_0}.
\]
The final inequality uses $n\ge T$ and $a\le a_0<T$.
...
The rounded comparisons in the main text follow from
Lemma~\ref{im:finite-list} and Proposition~\ref{prop:conditional-moments}. The following table records the integers
needed to check the three list examples. All have $k=n/2$.
\begin{center}
\begin{tabular}{rrrr}
\toprule
...
\midrule
 $2^{127}-1$ & 64 & 2 & 41\\
$2^{521}-1$ & 4096 & 8 & 3634\\
$2^{521}-1$ & 23444 & 45 & 11567\\
\bottomrule
\end{tabular}
\end{center}
For the first row the original denominator product is $32{,}285{,}041$,
giving $50{,}186{,}958{,}429$ codewords. The conditional dispersion
certificate~\eqref{eq:conditional-n64} strengthens this to
$2{,}674{,}048{,}230{,}075$, yielding the displayed exponent $41$.
For every row, exact integer arithmetic gives $p^s>2^n$, certifying
the sufficient below-Elias condition $\eta\log_2p>1$.
For the last row, a bound $2^{c_2/\eta}$ would require
\[
 c_2>\frac{11567\cdot45}{23444}>22.2.
\]
...
\section{Balanced rational fibers and the Frobenius index}
...
\paragraph{The collision components are saturated.}
The reduced curve $R(x)=R(y)$ in
$\mathbb P^1\times\mathbb P^1$ has bidegree $(B,B)$ and exactly
$nB$ points on $\mu_n^2$. All these points are smooth, since
each relevant fiber has $B$ distinct geometric preimages and hence
is unramified. Remove its $g$ deck graphs, where $g\ge1$ because
the diagonal is present. Each deck transformation preserves $\mu_n$:
every fiber meeting $\mu_n$ is already entirely contained in $\mu_n$.
Thus each graph contributes exactly $n$ subgroup points, and no two
components meet at any of these points.

Write $(a_i,b_i)$ for the bidegrees of the remaining components,
and $N_i$ for their subgroup-point counts. Neither projection is
constant. Projection to either coordinate gives
\[
 N_i\le n\min(a_i,b_i),\qquad
 \sum_i a_i=\sum_i b_i=B-g,\qquad
 \sum_i N_i=n(B-g).
\]
Equality in the sum forces $a_i=b_i$ and $N_i=na_i$ for every $i$.
Suppose such a component remains, and write $a=a_i\le B-1$.
Let $C$ be its smooth projective normalization, $K=k(C)=k(x,y)$,
and $S$ the union of the zeros and poles of $x$ and $y$.
The genus bound for a curve of bidegree $(a,a)$ gives
\begin{equation}\label{eq:frobenius-index-chi}
 g_C\le(a-1)^2,\qquad |S|\le4a,\qquad
 \chi:=2g_C-2+|S|\le2a^2.
\end{equation}

\paragraph{The four units.}
Put $u=x^n$, $v=y^n$, and $w=(u-1)/(v-1)$.
Because $\deg x=\deg y=a<p$, both coordinates are separating;
so are $u$ and $v$, since $p\nmid n$. In particular,
$[K^p(v):K^p]=p$. Outside $S$, $u-1$ and $v-1$ have exactly
the same zeros, all simple. Indeed, the complete fiber of $R(x)$
is contained in $\mu_n$ whenever $x\in\mu_n$, and both
projections of the collision curve are unramified there.
...
Either case makes the chosen component a deck graph, contrary to
...
\[
 ...
 \le 3(2g_C-2+|S|)=3\chi.
\]
The tuple contains a nonzero constant and $u$, so
$H\ge\deg u=na$. By~\eqref{eq:frobenius-index-chi},
$na\le6a^2$, contrary to $n>6(B-1)\ge6a$.
No component remains after the deck graphs are removed. The degree-$B$
map therefore has $B$ geometric deck transformations and is Galois.
\end{proof}
...
\input{research/power_family_mca/appendix.tex}
