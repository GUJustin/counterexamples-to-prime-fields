...
At message dimension three, a regular quadratic intersection bank and a
translated grid give a positive constant fraction of singleton exceptional
challenges over primes $p=\Theta(n^{5/4}\log n)$, with source separation
$\Theta(n^{1/4}/\log n)$ coordinates, in the regime of the
Dao--Kominers--Thaler bounds. The distance below capacity is
$\Theta(n^{-1/2})$ and the proximity loss is
$\Theta(n^{-3/4}/\log n)$. This is a vanishing-rate result, not
