...
\date{Working draft --- September 15, 2026}
\begin{document}
\maketitle
\begin{abstract}
...
polynomial spaces used by Circle STARKs. We apply the general results
to the quantitative list and line assumptions in the S-two whitepaper,
keeping the coding conclusions separate from claims about a complete
...
circle-code examples and apply them to the S-two analysis.
...
dimension $32$, Proposition~\ref{prop:polynomial-moment-weights}
proves a list of at least $5{,}285{,}900{,}426{,}578$ codewords with
$34$ agreements. Proposition~\ref{prop:moment-concentration} gives
...
$L\ge5285900426578>2^{42}$. Here $\eta=1/32$, so the choice
$c_1=c_2=1$ in \eqref{im:proposed-list} gives only $2^{32}$;
the improved ratio exceeds $1230$.
...
The finite Gram bound does not require $m$ to be fixed. To describe its
size, let $D_G=(m+2)\prod_{j=1}^m(2\lceil\sqrt{(m+2)V_j}\rceil+1)/2$
be the denominator in Proposition~\ref{prop:moment-concentration},
using the Gram variances. Uniformly when $t/n$ stays in a compact
subinterval of $(0,1)$ and $m\to\infty$ with $m\le\sqrt n$,
\[
 \log D_G=\frac{m^2}{2}\log(n/m)
 +(3/4-\log2)m^2+O(m\log n),
\]
where these logarithms are natural. This follows by inserting the exact
Gram norms and applying Stirling's bounds to
$\prod_{j=1}^m\sqrt{2j+1}(2j)!/j!$. The grid correction is
$O(m^4/n^2)$, and rounding contributes a smaller error because
$V_j\ge V_1=\Theta(n^3)$ throughout this range for sufficiently large $n$.
The complete-range denominator has the same expansion with $3/4$
in place of $3/4-\log2$. Thus, when $\log n=o(m)$, the Gram guarantee
improves the unrounded range guarantee by a factor
$2^{(1+o(1))m^2}$. This improves a lower-order term in the headline
regime; no growing-dimensional central limit theorem is used.
For example, exact arithmetic gives the following lower bounds on
$\lfloor\log_2 L\rfloor$ at rate $1/2$ and $t=n/2+m$:
\begin{center}
\begin{tabular}{rrrr}
\toprule
