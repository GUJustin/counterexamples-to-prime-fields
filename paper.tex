...
A separate low-rate bridge does enter the large-characteristic first-order
regime. Proposition~\ref{prop:low-rate-large-characteristic-singletons}
gives $N=p^5$, $K=p$, and more than $N^{6/5}$ exceptional challenges
over $\F_{p^{15}}$, with source common agreement $p^2-1$ and
actual agreement $p^3-1$. These are exactly all qualifying challenges
at that actual threshold, and each has a unique nearest codeword.
At the lower advertised threshold $p^3-p^2$, the same challenges lie
below Johnson and above the first-order curve, with an explicit uniform
$300000N^2$ upper comparison from the first-order counting formulas.
The rate and absolute margin both tend to zero; this gives a quantitative
large-characteristic comparison, not fixed-rate tightness or a prime-field
construction. The exceptional set and singleton lists are also exact at this
below-Johnson threshold. The full line has an exact three-level spectrum:
$M$ challenges attain $p^3-1$, another $M$ attain $p^2$, and every
remaining challenge attains $p^2-1$, with singleton nearest lists
at both higher levels.
