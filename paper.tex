...
tends to zero. The length can be $\Theta_\rho(\log p\log\log p)$.
...
and efficiently recoverable witnesses.
...
the code everywhere except at one point, and that point can be only
one coordinate short of being maximally far}. At every fixed rational
...
Theorems~\ref{pd:unique} and~\ref{pd:deterministic} have whole-line
...
Theorem~\ref{pd:deterministic} has an efficient witness-recovery algorithm. Lemma~\ref{pd:sampling} and Proposition~\ref{cw:sampling}
