...
one, even though the line contains a word outside the nearby radius,
with no correlated agreement. The number of coordinates by which that
word misses the radius can tend to infinity. At any desired fixed
nearby fraction below one, the outside distance can instead be a fixed
positive fraction of the capacity gap. An anchored lift and padding construction
gives failure of correlated agreement at an exactly fixed positive rate and gap, at
...
at half rate, more than $99.9999\%$ of a line can be nearby although
one point lies $\eta/7$ beyond the decoding radius and no common
agreement explanation exists (Theorem~\ref{fg:gap-scale-far}).
The proposed nearby fraction tends to zero. Thus being close to the
code for a random linear combination need not indicate common agreement
on a large coordinate set, even with an input separated from the radius
by a fixed fraction of the capacity gap. A different parameter choice
makes the nearby fraction tend to one, with an outside separation that
vanishes relative to $\eta$; the number of missing coordinates can still
grow (Corollary~\ref{fg:growing-far}).
...
can be. Theorem~\ref{fg:gap-scale-far} permits more than $99.9999\%$
nearby parameters at half rate, with a point $\eta/7$ outside the radius,
while the proposed numerical fraction tends to zero.
Theorem~\ref{fg:near-unit-density} makes the nearby fraction tend to one
with a smaller relative separation. These comparisons concern the
prescribed numerical coefficient; they do not replace the fixed-gap
argument needed to address a remainder controlled separately at each gap.
...
\item \textbf{Nearby density with separation proportional to the gap.}
At a fixed positive rate and fixed $\kappa>0$, can the nearby fraction tend to one while a
point of the line remains $\kappa\eta$ outside the decoding radius?
Theorem~\ref{fg:gap-scale-far} gives any desired fixed nearby fraction,
but its $\kappa$ depends on that fraction. The constructions with density
tending to one have separation $o(\eta)$. This question concerns shrinking
gaps on short prime-field domains, separately from the fixed-gap problem.
At vanishing rate, Theorem~\ref{fg:punctured-line} already gives exactly
one far point and all nonzero parameters nearby, with separation
proportional to $\eta$.
